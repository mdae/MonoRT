...
namespace TimingTest
{
	class Timing1000Methods
	{
		...
		static int Main (string[] args)
		{
			...
			StreamReader StartTime = File.OpenText ("./starttime.txt");
			shelltime = StartTime.ReadLine ();
			StartTime.Close ();
			
			//Console.WriteLine("args[1]: " + args[1]);
			strtIndx = shelltime.IndexOf("::");
			//
			//  Bestimmung des Sekunden-Anteils des per Kommandozeile uebergebenen Arguments
			//
			seconds = shelltime.Substring(0, strtIndx);
			//
			//  Bestimmung der Nanosekunden-Anteils des per Kommandozeile uebergebenen Arguments
			//
			nanoseconds = shelltime.Substring(strtIndx + "::".Length);
