...
mono_dl_open (const char *name, int flags, char **error_msg)
{
	...
	//printf("Flags 1: %i\n", lflags);
	int lflags = RTLD_GLOBAL | RTLD_NOW | RTLD_DEEPBIND;

	//printf("Flags 2\tRTLD_GLOBAL: %i\tRTLD_NOW: %i\RTLD_DEEPBIND: %i\n", (int)RTLD_GLOBAL, (int) RTLD_NOW, (int)RTLD_DEEPBIND);
	//printf("dlopen(): Name: %s\tFlags: %i\n",((name == NULL) ? "-" : name), (int) lflags);
	//fflush(NULL);
	//
