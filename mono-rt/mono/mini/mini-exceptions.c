		    ...
		    gboolean *managed)
{
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	//  Die Erzeugung der Stackframes fuer das Monitoring
	//  des JIT-Compilers und der Trampoline verursacht
	//  gelegentlich einen Segmentation Fault in dieser
	//  Funktion. Dieser Fall wird nun gefilter und es
	//  wird kein Stackframe ausgegeben.
	//
	if ((!managed2) && (ji->method == NULL)) {
		printf("\n\t*** StackTrace abgebrochen ***\n\n");
		fflush(NULL);
		return (gpointer)-1;
	}
	//
...
void
mono_jit_walk_stack_from_ctx_in_thread (MonoJitStackWalk func, MonoDomain *domain, MonoContext *start_ctx, gboolean do_il_offset, MonoThread *thread, MonoLMF *lmf, gpointer user_data)
{
	...
	if (start_ctx) {
		...
		MONO_INIT_CONTEXT_FROM_FUNC (&ctx, mono_jit_walk_stack_from_ctx);
#endif
		g_assert (thread == mono_thread_current ());
	}
...
mono_altstack_restore_prot (mgreg_t *regs, guint8 *code, gpointer *tramp_data, guint8* tramp)
{
	...
	{
		ret = asprintf(&messageToUser,"MonoRT - mono_altstack_restore_prot ():\n\tThread: %lu\n\tAppDomain: %u\n",
