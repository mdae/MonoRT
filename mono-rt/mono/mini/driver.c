	...
	for (opt = 0; opt < G_N_ELEMENTS (opt_sets); ++opt) {
		...
		for (i = 0; i < mono_image_get_table_rows (image, MONO_TABLE_METHOD); ++i) {
			...
			if (strncmp (method->name, "test_", 5) == 0) {
				...
				cfg = mini_method_compile (method, opt_flags, mono_get_root_domain (), TRUE, FALSE, 0, 0);
				//
				//  Ende Pre-JIT/-Patch Code
				//
...
compile_all_methods_thread_main (CompileAllThreadArgs *args)
{
	...
	for (i = 0; i < mono_image_get_table_rows (image, MONO_TABLE_METHOD); ++i) {
		...
		cfg = mini_method_compile (method, args->opts, mono_get_root_domain (), FALSE, FALSE, 0, 0);
		//
		//  Ende Pre-JIT/-Patch Code
		//
		if (cfg->exception_type != MONO_EXCEPTION_NONE) {
...
static void main_thread_handler (gpointer user_data)
{
	...
	if (mono_compile_aot) {
		...
		//  Beginn Pre-JIT/-Patch Code
		//
		doPrePatch_g = doPrePatchCmd_g;
		preJitCompile (assembly, main_args);
		initializeAndStartPrePatch (main_args);
		//
		...
		if (0 != doWaitForChkPoint_g)
		{
			int i;
			doWaitForChkPoint_g = 0;
			for (i = 30; i > 0; i--)
			{
				printf("%i seconds to start\n", i);
				sleep(1);
			}
		}
		//
...
mono_main (int argc, char* argv[])
{
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	//  Initialisierung eigener Datenstrukturen
	//
	initPrePatchDataStructures();
	char * delegateOffset;
	//
	//  Ende Pre-JIT/-Patch Code
	//
	...
	for (i = 1; i < argc; ++i) {
		...
		if (strcmp (argv [i], "--regression") == 0) {
			...
			mini_verbose++;
		}else if (strcmp (argv [i], "--rtgc-dbg") == 0){ 
			 mono_rtgc_dbg=1;

		} else if (strcmp (argv [i], "--version") == 0 || strcmp (argv [i], "-V") == 0) {
			char *build = mono_get_runtime_build_info ();
			g_print ("Mono Real-Time based on Mono JIT compiler version %s\nCopyright (C) 2002-2008 Novell, Inc and Contributors. www.mono-project.com\n", build);
			g_free (build);
			g_print (info);
			if (mini_verbose) {
				const char *cerror;
		...
		else if (strcmp (argv [i], "--snoop") == 0)
		{
			doTrace_g = 1;
		}
		else if (strcmp (argv [i], "--snoop2") == 0)
		{
			doTraceLevel2_g = 1;
		}
		else if (strcmp (argv [i], "--snoopcan") == 0)
		{
			 snoopcan = 1;
		}
		else if (strcmp (argv [i], "--snoopcap") == 0)
		{
			 snoopcap = 1;
		}
		else if (strcmp (argv [i], "--snooptc") == 0)
		{
			//
			//  Aktivierung der Debug-Ausgaben der Initialisierung
			//  der Arrays der Standardklassen Monos.
			//
			snooptc = 1;
		}
		else if (strcmp (argv [i], "--jtrace") == 0)
		{
			//
			//  Aktivierung der Aktivitaetsanzeige des JIT-Compilers
			//  waehrend der Laufzeit (fuer Debugging-Zwecke).
			//
			doTraceJITCmd_g = 1;
		}
		...
		{
			//
			//  Aktivierung der Aktivitaetsanzeige diverser Trampoline
			//  waehrend der Laufzeit (fuer Debugging-Zwecke).
			//
			doTracePatchCmd_g = 1;
		}
		else if (strncmp (argv [i], "--asmtrace=", 11) == 0)
		{
			//
			//  Aktivierung der Ausgabe des nativen Codes
			//  angegebener Methoden.
			//
			snoopasm = 1;
			parseMethodsForAsmTrace (argv [i]);
		} else if (strcmp (argv [i], "--snooptramp") == 0) {
			 snooptramp = 1;
		} else if (strcmp (argv [i], "--snoopcv") == 0) {
			 snoopcallvirt = 1;
		} else if (strcmp (argv [i], "--snoopvt") == 0) {
			 snoopvtpp = 1;
		} else if (strcmp (argv [i], "--wtrace") == 0) {
			 snoopwrapper_org = 1;
		} else if (strcmp (argv [i], "--prepatch") == 0) {
			//
			//  Es werden alle Pre-Patch-Funktionen aktiviert
			//
			doPrePatchCmd_g |= PREPATCHALL;
			doPreJIT_g = 1;
		} else if (strcmp (argv [i], "--pp1") == 0) {
			//
			//  Es wird nur das Pre-Patch der direkten
			//  Call-Anweisungen aktiviert.
			//
			doPrePatchCmd_g |= PREPATCHCALLS;
			doPreJIT_g = 1;
		} else if (strcmp (argv [i], "--pp2") == 0) {
			//
			//  Es wird nur das Pre-Patch der VTable/IMT-Calĺs
			//  aktiviert.
			//
			doPrePatchCmd_g |= PREPATCHVIRT;
			doPreJIT_g = 1;
		} else if (strcmp (argv [i], "--pp3") == 0) {
			//
			//  Es wird das Pre-Patch der Delegate-Calls
			//  aktiviert. Beim wiederholten Betreten des
			//  Delegate Trampoline wird immer in das
			//  modifizierte Generic Delegate Trampoline
			//  zurückgesprungen.
			//
			doPrePatchCmd_g |= PREPATCHDEL;
			doPreJIT_g = 1;
		} else if (strcmp (argv [i], "--pp4") == 0) {
			//
			//  Es wird das Pre-Patch der Delegate-Calls
			//  aktiviert. Beim wiederholten Betreten eines
			//  Delegate Trampoline wird in ein nicht-modifiziertes
			//  Generic Delegate Trampoline zurueckgesprungen.
			//
			doPrePatchCmd_g |= PREPATCHDELEXT;
			doPreJIT_g = 1;
		} else if (strcmp (argv [i], "--patchlist") == 0) {
			//
			//  Aktiviert die Ausgabe der aufgezeichneten
			//  Trampoline, VTables, Klassen, Methoden, etc.
			//
			patchlist = 1;
		} else if (strcmp (argv [i], "--prejit") == 0) {
			//
			//  Aktiviert das Pre-JIT
			//
			doPreJIT_g = 1;
		} else if (strcmp (argv [i], "--checkST") == 0) {
			//
			//  Aktiviert die Ausfuehrung der Funktion
			//  'validateSpecificTrampolineJumpTarget()'. Falls
			//  Probleme beim Pre-Patch direkter Call-Instruktionen
			//  auftreten, sollte diese Option gesetzt werden.
			//
			validateSpecificTrampoline_g = 1;
		} else if (strcmp (argv [i], "--mtsp") == 0) {
			//
			//  Zwingt die Monitor-Enter/Exit-Trampoline stets
			//  den Slow-Path zu verwenden.
			//
			useSlowPath_g = 1;
		} else if (strcmp (argv [i], "--disablegc") == 0) {
			 disablegc = 1;
		} else if (strcmp (argv [i], "--wait") == 0) {
			doWaitForChkPoint_g = 1;
		//
		//  Ende Pre-JIT/-Patch Code
		//
		} else {
...
#else
		printf("\n\t**** Der MDB wird nicht unterstuetzt ****\n");
		fflush(NULL);
	 ...
	 * Since 
	 () is hopefully only used by the runtime executable, this 
	...
	if (action == DO_DRAW) {
		...
		if ((method->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) ||
			...
			cfg = mini_method_compile (nm, opt, mono_get_root_domain (), FALSE, FALSE, part, 0);
			//
			//  Ende Pre-JIT/-Patch Code
			//
		}
			...
			cfg = mini_method_compile (method, opt, mono_get_root_domain (), FALSE, FALSE, part, 0);
			//
			//  Ende Pre-JIT/-Patch Code
			//
		...
		if (mini_stats_fd) {
			...
			for (i = 0; i < G_N_ELEMENTS (opt_sets); i++) {
				...
				for (j = 0; j < count; ++j) {
					...
					cfg = mini_method_compile (method, opt, mono_get_root_domain (), FALSE, FALSE, 0, 0);
					//
					//  Ende Pre-JIT/-Patch Code
					//
					mono_destroy_compile (cfg);
