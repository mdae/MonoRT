...
resolve_patch (MonoCompile *cfg, MonoJumpInfoType type, gconstpointer target)
{
	...
	return mono_resolve_patch_target (cfg->method, cfg->domain, NULL, &ji, FALSE, 0);
	//
			...
			case OP_VCALL_MEMBASE: {
				...
				if (ins->flags & MONO_INST_HAS_METHOD) {
					...
					if (info) {
						...
						target = mono_resolve_patch_target (cfg->method, cfg->domain, NULL, &ji, FALSE, 0);
						//
						//  Ende Pre-JIT/-Patch Code
						//
						*/
						...
						if (cfg->compile_aot) {
							...
							if (cfg->abs_patches) {
								...
								if (abs_ji) {
									...
									target = mono_resolve_patch_target (cfg->method, cfg->domain, NULL, abs_ji, FALSE, 0);
									//
									//  Ende Pre-JIT/-Patch Code
									//
									LLVMAddGlobalMapping (ee, callee, target);
