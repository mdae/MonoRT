...
MonoInst* mono_emit_native_call(MonoCompile *cfg, gconstpointer func,
		MonoMethodSignature *sig, MonoInst **args);
void mini_emit_stobj(MonoCompile *cfg, MonoInst *dest, MonoInst *src,
		MonoClass *klass, gboolean native);
void mini_emit_initobj(MonoCompile *cfg, MonoInst *dest, const guchar *ip,
		MonoClass *klass);
...
void mono_inst_set_src_registers(MonoInst *ins, int *regs) {
	ins->sreg1 = regs[0];
	ins->sreg2 = regs[1];
	ins->sreg3 = regs[2];
}
 ...
 */
static void link_bblock(MonoCompile *cfg, MonoBasicBlock *from,
		MonoBasicBlock* to) {
	MonoBasicBlock **newa;
	int i, found;
	...
	if (!found) {
		newa = mono_mempool_alloc(cfg->mempool, sizeof(gpointer)
				* (from->out_count + 1));
		for (i = 0; i < from->out_count; ++i) {
...
void mono_link_bblock(MonoCompile *cfg, MonoBasicBlock *from,
		MonoBasicBlock* to) {
 ...
 */
static int mono_find_block_region(MonoCompile *cfg, int offset) {
	...
	for (i = 0; i < header->num_clauses; ++i) {
		...
		if ((clause->flags == MONO_EXCEPTION_CLAUSE_FILTER) && (offset
				>= clause->data.filter_offset) && (offset
...
static GList*
mono_find_final_block(MonoCompile *cfg, unsigned char *ip,
		unsigned char *target, int type) {
	MonoMethod *method = cfg->method;
	MonoMethodHeader *header = mono_method_get_header(method);
	MonoExceptionClause *clause;
...
static void mono_create_spvar_for_region(MonoCompile *cfg, int region) {
	MonoInst *var;
	...
	var = mono_compile_create_var(cfg, &mono_defaults.int_class->byval_arg,
			OP_LOCAL);
...
mono_create_exvar_for_offset(MonoCompile *cfg, int offset) {
	...
	var = mono_compile_create_var(cfg, &mono_defaults.object_class->byval_arg,
			OP_LOCAL);
	/* prevent it from being register allocated */
	var->flags |= MONO_INST_INDIRECT;
 ...
 */
static const char bin_num_table[STACK_MAX][STACK_MAX] = { { STACK_INV,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
		STACK_INV }, { STACK_INV, STACK_I4, STACK_INV, STACK_PTR, STACK_INV,
		STACK_MP, STACK_INV, STACK_INV }, { STACK_INV, STACK_INV, STACK_I8,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV }, { STACK_INV,
		STACK_PTR, STACK_INV, STACK_PTR, STACK_INV, STACK_MP, STACK_INV,
		STACK_INV }, { STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_R8,
		STACK_INV, STACK_INV, STACK_INV }, { STACK_INV, STACK_MP, STACK_INV,
		STACK_MP, STACK_INV, STACK_PTR, STACK_INV, STACK_INV }, { STACK_INV,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
		STACK_INV }, { STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
		STACK_INV, STACK_INV, STACK_INV } };

static const char neg_table[] = { STACK_INV, STACK_I4, STACK_I8, STACK_PTR,
		STACK_R8, STACK_INV, STACK_INV, STACK_INV };

/* reduce the size of this table */
static const char bin_int_table[STACK_MAX][STACK_MAX] = { { STACK_INV,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
		STACK_INV }, { STACK_INV, STACK_I4, STACK_INV, STACK_PTR, STACK_INV,
		STACK_INV, STACK_INV, STACK_INV }, { STACK_INV, STACK_INV, STACK_I8,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV }, { STACK_INV,
		STACK_PTR, STACK_INV, STACK_PTR, STACK_INV, STACK_INV, STACK_INV,
		STACK_INV }, { STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
		STACK_INV, STACK_INV, STACK_INV }, { STACK_INV, STACK_INV, STACK_INV,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV }, { STACK_INV,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
...
static const char bin_comp_table[STACK_MAX][STACK_MAX] = {
/*	Inv i  L  p  F  &  O  vt */
{ 0 }, { 0, 1, 0, 1, 0, 0, 0, 0 }, /* i, int32 */
...
/* reduce the size of this table */
static const char shift_table[STACK_MAX][STACK_MAX] = { { STACK_INV, STACK_INV,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV }, {
		STACK_INV, STACK_I4, STACK_INV, STACK_I4, STACK_INV, STACK_INV,
		STACK_INV, STACK_INV }, { STACK_INV, STACK_I8, STACK_INV, STACK_I8,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV }, { STACK_INV, STACK_PTR,
		STACK_INV, STACK_PTR, STACK_INV, STACK_INV, STACK_INV, STACK_INV }, {
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
		STACK_INV, STACK_INV }, { STACK_INV, STACK_INV, STACK_INV, STACK_INV,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV }, { STACK_INV, STACK_INV,
		STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV }, {
...
static const guint16 binops_op_map[STACK_MAX] = { 0, OP_IADD - CEE_ADD, OP_LADD
		- CEE_ADD, OP_PADD - CEE_ADD, OP_FADD - CEE_ADD, OP_PADD - CEE_ADD };

/* handles from CEE_NEG to CEE_CONV_U8 */
static const guint16 unops_op_map[STACK_MAX] = { 0, OP_INEG - CEE_NEG, OP_LNEG
		- CEE_NEG, OP_PNEG - CEE_NEG, OP_FNEG - CEE_NEG, OP_PNEG - CEE_NEG };

/* handles from CEE_CONV_U2 to CEE_SUB_OVF_UN */
static const guint16 ovfops_op_map[STACK_MAX] = { 0, OP_ICONV_TO_U2
		- CEE_CONV_U2, OP_LCONV_TO_U2 - CEE_CONV_U2, OP_PCONV_TO_U2
		- CEE_CONV_U2, OP_FCONV_TO_U2 - CEE_CONV_U2, OP_PCONV_TO_U2
		- CEE_CONV_U2, OP_PCONV_TO_U2 - CEE_CONV_U2 };

/* handles from CEE_CONV_OVF_I1_UN to CEE_CONV_OVF_U_UN */
static const guint16 ovf2ops_op_map[STACK_MAX] = { 0, OP_ICONV_TO_OVF_I1_UN
		- CEE_CONV_OVF_I1_UN, OP_LCONV_TO_OVF_I1_UN - CEE_CONV_OVF_I1_UN,
		OP_PCONV_TO_OVF_I1_UN - CEE_CONV_OVF_I1_UN, OP_FCONV_TO_OVF_I1_UN
				- CEE_CONV_OVF_I1_UN, OP_PCONV_TO_OVF_I1_UN
				- CEE_CONV_OVF_I1_UN };

/* handles from CEE_CONV_OVF_I1 to CEE_CONV_OVF_U8 */
static const guint16 ovf3ops_op_map[STACK_MAX] = { 0, OP_ICONV_TO_OVF_I1
		- CEE_CONV_OVF_I1, OP_LCONV_TO_OVF_I1 - CEE_CONV_OVF_I1,
		OP_PCONV_TO_OVF_I1 - CEE_CONV_OVF_I1, OP_FCONV_TO_OVF_I1
				- CEE_CONV_OVF_I1, OP_PCONV_TO_OVF_I1 - CEE_CONV_OVF_I1 };

/* handles from CEE_BEQ to CEE_BLT_UN */
static const guint16 beqops_op_map[STACK_MAX] = { 0, OP_IBEQ - CEE_BEQ, OP_LBEQ
		- CEE_BEQ, OP_PBEQ - CEE_BEQ, OP_FBEQ - CEE_BEQ, OP_PBEQ - CEE_BEQ,
		OP_PBEQ - CEE_BEQ };

/* handles from CEE_CEQ to CEE_CLT_UN */
static const guint16 ceqops_op_map[STACK_MAX] = { 0, OP_ICEQ - OP_CEQ, OP_LCEQ
		- OP_CEQ, OP_PCEQ - OP_CEQ, OP_FCEQ - OP_CEQ, OP_PCEQ - OP_CEQ, OP_PCEQ
		- OP_CEQ };
 ...
 */
static void type_from_op(MonoInst *ins, MonoInst *src1, MonoInst *src2) {
	...
	case OP_LCOMPARE:
	case OP_ICOMPARE:
		ins->type = bin_comp_table[src1->type][src2->type] ? STACK_I4
				: STACK_INV;
		if ((src1->type == STACK_I8) || ((SIZEOF_REGISTER == 8) && ((src1->type
				== STACK_PTR) || (src1->type == STACK_OBJ) || (src1->type
				== STACK_MP))))
	...
	case OP_CLT:
	case OP_CLT_UN:
		ins->type = (bin_comp_table[src1->type][src2->type] & 1) ? STACK_I4
				: STACK_INV;
		ins->opcode += ceqops_op_map[src1->type];
		break;
		/* unops */
...
static const char
		ldind_type[] = { STACK_I4, STACK_I4, STACK_I4, STACK_I4, STACK_I4,
				STACK_I4, STACK_I8, STACK_PTR, STACK_R8, STACK_R8, STACK_OBJ };
...
mono_get_domainvar(MonoCompile *cfg) {
	if (!cfg->domainvar)
		cfg->domainvar = mono_compile_create_var(cfg,
...
mono_get_got_var(MonoCompile *cfg) {
	...
	if (!cfg->compile_aot)
		return NULL;
	if (!cfg->got_var) {
		cfg->got_var = mono_compile_create_var(cfg,
				&mono_defaults.int_class->byval_arg, OP_LOCAL);
	}
	return cfg->got_var;
#else
...
mono_get_vtable_var(MonoCompile *cfg) {
	...
	if (!cfg->rgctx_var) {
		cfg->rgctx_var = mono_compile_create_var(cfg,
				&mono_defaults.int_class->byval_arg, OP_LOCAL);
...
static void mono_save_token_info(MonoCompile *cfg, MonoImage *image,
		guint32 token, gpointer key) {
	/* 
	...
	if (cfg->compile_aot && !cfg->generic_context
			&& (mono_metadata_token_table (token) > 0)) {
		MonoJumpInfoToken *jump_info_token = mono_mempool_alloc0(cfg->mempool,
 ...
 */
static void handle_stack_args(MonoCompile *cfg, MonoInst **sp, int count) {
	int i, bindex;
	...
	if (!bb->out_scount) {
		...
		//printf ("\n");
		if (!found) {
			bb->out_stack = mono_mempool_alloc(cfg->mempool, sizeof(MonoInst*)
					* count);
			for (i = 0; i < count; ++i) {
				/* 
				...
				if (cfg->inlined_method)
					bb->out_stack[i] = mono_compile_create_var(cfg,
							type_from_stack_type(sp[i]), OP_LOCAL);
				else
					bb->out_stack[i] = mono_compile_get_interface_var(cfg, i,
							sp[i]);
			}
		}
	}
 ...
 */
static void mini_emit_load_intf_reg_vtable(MonoCompile *cfg, int intf_reg,
		int vtable_reg, MonoClass *klass) {
	if (cfg->compile_aot) {
 ...
 */
static void mini_emit_load_intf_bit_reg_class(MonoCompile *cfg,
		int intf_bit_reg, int klass_reg, MonoClass *klass) {
 ...
 */
static void mini_emit_load_intf_bit_reg_vtable(MonoCompile *cfg,
		int intf_bit_reg, int vtable_reg, MonoClass *klass) {
	int ibitmap_reg = alloc_preg(cfg);
	int ibitmap_byte_reg = alloc_preg(cfg);
 ...
 */
static void mini_emit_max_iid_check(MonoCompile *cfg, int max_iid_reg,
		MonoClass *klass, MonoBasicBlock *false_target) {
	if (cfg->compile_aot) {
		int iid_reg = alloc_preg(cfg);
		MONO_EMIT_NEW_AOTCONST (cfg, iid_reg, klass, MONO_PATCH_INFO_IID);
...
static void mini_emit_max_iid_check_vtable(MonoCompile *cfg, int vtable_reg,
		MonoClass *klass, MonoBasicBlock *false_target) {
...
static void mini_emit_max_iid_check_class(MonoCompile *cfg, int klass_reg,
		MonoClass *klass, MonoBasicBlock *false_target) {
	int max_iid_reg = alloc_preg(cfg);
...
static void mini_emit_isninst_cast(MonoCompile *cfg, int klass_reg,
		MonoClass *klass, MonoBasicBlock *false_target,
		MonoBasicBlock *true_target) {
...
static void mini_emit_iface_cast(MonoCompile *cfg, int vtable_reg,
		MonoClass *klass, MonoBasicBlock *false_target,
		MonoBasicBlock *true_target) {
 ...
 */
static void mini_emit_iface_class_cast(MonoCompile *cfg, int klass_reg,
		MonoClass *klass, MonoBasicBlock *false_target,
		MonoBasicBlock *true_target) {
...
static inline void mini_emit_class_check(MonoCompile *cfg, int klass_reg,
		MonoClass *klass) {
	if (cfg->compile_aot) {
		int const_reg = alloc_preg(cfg);
		MONO_EMIT_NEW_CLASSCONST (cfg, const_reg, klass);
...
static inline void mini_emit_class_check_branch(MonoCompile *cfg,
		int klass_reg, MonoClass *klass, int branch_op, MonoBasicBlock *target) {
	if (cfg->compile_aot) {
		int const_reg = alloc_preg(cfg);
		MONO_EMIT_NEW_CLASSCONST (cfg, const_reg, klass);
		MONO_EMIT_NEW_BIALU (cfg, OP_COMPARE, -1, klass_reg, const_reg);
...
static void mini_emit_castclass(MonoCompile *cfg, int obj_reg, int klass_reg,
		MonoClass *klass, MonoBasicBlock *object_is_null) {
	if (klass->rank) {
		...
		if (klass->cast_class == mono_defaults.object_class) {
			...
			mini_emit_class_check_branch(cfg, parent_reg,
					mono_defaults.enum_class->parent, OP_PBNE_UN,
					object_is_null);
			mini_emit_class_check(cfg, eclass_reg, mono_defaults.enum_class);
		} else if (klass->cast_class == mono_defaults.enum_class->parent) {
			mini_emit_class_check_branch(cfg, eclass_reg,
					mono_defaults.enum_class->parent, OP_PBEQ, object_is_null);
			mini_emit_class_check(cfg, eclass_reg, mono_defaults.enum_class);
		} else if (klass->cast_class == mono_defaults.enum_class) {
			mini_emit_class_check(cfg, eclass_reg, mono_defaults.enum_class);
		} else if (klass->cast_class->flags & TYPE_ATTRIBUTE_INTERFACE) {
			mini_emit_iface_class_cast(cfg, eclass_reg, klass->cast_class,
					NULL, NULL);
		} else {
			// Pass -1 as obj_reg to skip the check below for arrays of arrays
			mini_emit_castclass(cfg, -1, eclass_reg, klass->cast_class,
					object_is_null);
		}
...
static void mini_emit_memset(MonoCompile *cfg, int destreg, int offset,
		int size, int val, int align) {
	int val_reg;
...
void mini_emit_memcpy(MonoCompile *cfg, int destreg, int doffset, int srcreg,
		int soffset, int size, int align) {
	int cur_reg;
...
static int ret_type_to_call_opcode(MonoType *type, int calli, int virt,
		MonoGenericSharingContext *gsctx) {
	if (type->byref)
		return calli ? OP_CALL_REG : virt ? OP_CALLVIRT : OP_CALL;

	handle_enum: type = mini_get_basic_type_from_generic(gsctx, type);
	switch (type->type) {
	case MONO_TYPE_VOID:
 ...
 */
static int target_type_is_incompatible(MonoCompile *cfg, MonoType *target,
		MonoInst *arg) {
	...
	case MONO_TYPE_PTR:
		...
		if (arg->type != STACK_I4 && arg->type != STACK_PTR && arg->type
				!= STACK_MP)
			return 1;
		return 0;
	case MONO_TYPE_I:
	...
	case MONO_TYPE_MVAR:
		...
		return 0;
	default:
		g_error("unknown type 0x%02x in target_type_is_incompatible",
				simple_type->type);
	}
	return 1;
}
 ...
 */
static int check_call_signature(MonoCompile *cfg, MonoMethodSignature *sig,
		MonoInst **args) {
	MonoType *simple_type;
	int i;

	if (sig->hasthis) {
		if (args[0]->type != STACK_OBJ && args[0]->type != STACK_MP
				&& args[0]->type != STACK_PTR)
			return 1;
		args++;
	}
	for (i = 0; i < sig->param_count; ++i) {
		...
		simple_type = mini_get_basic_type_from_generic(
				cfg->generic_sharing_context, simple_type);
		handle_enum: switch (simple_type->type) {
		case MONO_TYPE_VOID:
			return 1;
			continue;
		case MONO_TYPE_I1:
		...
		case MONO_TYPE_FNPTR:
			if (args[i]->type != STACK_I4 && args[i]->type != STACK_PTR
					&& args[i]->type != STACK_MP && args[i]->type != STACK_OBJ)
				return 1;
			continue;
		case MONO_TYPE_CLASS:
		case MONO_TYPE_STRING:
		...
		case MONO_TYPE_GENERICINST:
			simple_type
					= &simple_type->data.generic_class->container_class->byval_arg;
			goto handle_enum;
...
static int callvirt_to_call(int opcode) {
	switch (opcode) {
	case OP_CALLVIRT:
		return OP_CALL;
...
static int callvirt_to_call_membase(int opcode) {
	switch (opcode) {
	case OP_CALLVIRT:
...
static void emit_imt_argument(MonoCompile *cfg, MonoCallInst *call, MonoInst *imt_arg) {
...
static MonoJumpInfo *
mono_patch_info_new(MonoMemPool *mp, int ip, MonoJumpInfoType type,
...
#else
	//
	// für iMethod wird dieser Pfad betreten
	//
	mono_arch_emit_call(cfg, call);
#endif
...
inline static MonoInst*
mono_emit_calli(MonoCompile *cfg, MonoMethodSignature *sig, MonoInst **args,
		MonoInst *addr) {
	MonoCallInst *call =
			mono_emit_call_args(cfg, sig, args, TRUE, FALSE, FALSE);
...
mono_emit_rgctx_calli(MonoCompile *cfg, MonoMethodSignature *sig,
		MonoInst **args, MonoInst *addr, MonoInst *rgctx_arg) {
...
static MonoInst*
emit_get_rgctx_method(MonoCompile *cfg, int context_used, MonoMethod *cmethod,
		int rgctx_type);
static MonoInst*
emit_get_rgctx_klass(MonoCompile *cfg, int context_used, MonoClass *klass,
		int rgctx_type);

static MonoInst*
mono_emit_method_call_full(MonoCompile *cfg, MonoMethod *method,
		MonoMethodSignature *sig, MonoInst **args, MonoInst *this,
		MonoInst *imt_arg) {
	//
	// für iMethod wird dieser Pfad betreten
	//
	...
	if (method->string_ctor) {
		...
		/* FIXME: Cache these */
		MonoMethodSignature *ctor_sig = mono_metadata_signature_dup_mempool(
				cfg->mempool, sig);
	...
	might_be_remote = this && sig->hasthis && (method->klass->marshalbyref
			|| method->klass == mono_defaults.object_class) && !(method->flags
			& METHOD_ATTRIBUTE_VIRTUAL) && !MONO_CHECK_THIS (this);
	//
	// für iMethod might_be_remote == 0
	//
	...
	if (virtual) {
		...
		if ((!cfg->compile_aot || enable_for_aot) && (!(method->flags & METHOD_ATTRIBUTE_VIRTUAL) || (MONO_METHOD_IS_FINAL (method)
		...
		call->inst.opcode = callvirt_to_call_membase(call->inst.opcode);
		//
		// für iMethod wird OP_VOIDCALL_MEMBASE gewählt
		//
		...
		if (method->klass->flags & TYPE_ATTRIBUTE_INTERFACE) {
			...
			if (mono_use_imt) {
				...
				slot_reg = vtable_reg;
				call->inst.inst_offset = ((gint32) imt_slot - MONO_IMT_SIZE)
						* SIZEOF_VOID_P;
			}
#endif
			if (slot_reg == -1) {
				...
				mini_emit_load_intf_reg_vtable(cfg, slot_reg, vtable_reg,
						method->klass);
				call->inst.inst_offset = mono_method_get_vtable_index(method)
						* SIZEOF_VOID_P;
			}
		} else {
...
static MonoInst*
mono_emit_rgctx_method_call_full(MonoCompile *cfg, MonoMethod *method,
		MonoMethodSignature *sig, MonoInst **args, MonoInst *this,
		MonoInst *imt_arg, MonoInst *vtable_arg) {
	int rgctx_reg = 0;
	MonoInst *ins;
	MonoCallInst *call;
...
mono_emit_method_call(MonoCompile *cfg, MonoMethod *method, MonoInst **args,
		MonoInst *this) {
	return mono_emit_method_call_full(cfg, method,
			mono_method_signature(method), args, this, NULL);
}
...
mono_emit_jit_icall(MonoCompile *cfg, gconstpointer func, MonoInst **args) {
	...
	return mono_emit_native_call(cfg, mono_icall_get_wrapper(info), info->sig,
			args);
}
 ...
 */
inline static MonoInst*
mono_emit_abs_call(MonoCompile *cfg, MonoJumpInfoType patch_type,
		gconstpointer data, MonoMethodSignature *sig, MonoInst **args) {
...
static MonoInst*
mono_emit_widen_call_res(MonoCompile *cfg, MonoInst *ins,
...
get_memcpy_method(void) {
	...
	if (!memcpy_method) {
		memcpy_method = mono_class_get_method_from_name(
				mono_defaults.string_class, "memcpy", 3);
		if (!memcpy_method)
...
get_memset_method(void) {
	...
	if (!memset_method) {
		memset_method = mono_class_get_method_from_name(
				mono_defaults.string_class, "memset", 3);
		if (!memset_method)
			g_error("Old corlib found. Install a new one");
	}
...
emit_get_rgctx(MonoCompile *cfg, MonoMethod *method, int context_used) {
	...
	if (!(method->flags & METHOD_ATTRIBUTE_STATIC) && !(context_used
			& MONO_GENERIC_CONTEXT_USED_METHOD) && !method->klass->valuetype)
...
mono_patch_info_rgctx_entry_new(MonoMemPool *mp, MonoMethod *method,
		gboolean in_mrgctx, MonoJumpInfoType patch_type,
		gconstpointer patch_data, int info_type) {
	MonoJumpInfoRgctxEntry *res = mono_mempool_alloc0(mp,
...
static inline MonoInst* emit_rgctx_fetch(MonoCompile *cfg, MonoInst *rgctx, MonoJumpInfoRgctxEntry *entry) {

	return mono_emit_abs_call(cfg, MONO_PATCH_INFO_RGCTX_FETCH, entry, helper_sig_rgctx_lazy_fetch_trampoline, &rgctx);
}

static MonoInst* emit_get_rgctx_klass(MonoCompile *cfg, int context_used, MonoClass *klass, int rgctx_type) {
...
static MonoInst* emit_get_rgctx_method(MonoCompile *cfg, int context_used, MonoMethod *cmethod,	int rgctx_type) {
...
static MonoInst*
emit_get_rgctx_field(MonoCompile *cfg, int context_used, MonoClassField *field,
		int rgctx_type) {
	MonoJumpInfoRgctxEntry *entry = mono_patch_info_rgctx_entry_new(
			cfg->mempool, cfg->current_method, context_used
					& MONO_GENERIC_CONTEXT_USED_METHOD, MONO_PATCH_INFO_FIELD,
			field, rgctx_type);
	MonoInst *rgctx = emit_get_rgctx(cfg, cfg->current_method, context_used);
 ...
 */
static void emit_generic_class_init(MonoCompile *cfg, MonoClass *klass) {
	MonoInst *vtable_arg;
	MonoCallInst *call;
	int context_used = 0;
	...
	if (context_used) {
		vtable_arg = emit_get_rgctx_klass(cfg, context_used, klass,
	...
	mono_call_inst_add_outarg_reg(cfg, call, vtable_arg->dreg,
			MONO_ARCH_VTABLE_REG, FALSE);
 ...
 */
static void mini_emit_check_array_type(MonoCompile *cfg, MonoInst *obj,
	...
	if (cfg->opt & MONO_OPT_SHARED) {
		...
		vtable_ins = emit_get_rgctx_klass(cfg, context_used, array_class,
				MONO_RGCTX_INFO_VTABLE);
...
static void save_cast_details(MonoCompile *cfg, MonoClass *klass, int obj_reg) {
...
static void reset_cast_details(MonoCompile *cfg) {
...
handle_unbox_nullable(MonoCompile* cfg, MonoInst* val, MonoClass* klass,
	...
	if (context_used) {
		...
		return mono_emit_rgctx_calli(cfg, mono_method_signature(method), &val,
				addr, rgctx);
	} else {
...
handle_alloc(MonoCompile *cfg, MonoClass *klass, gboolean for_box) {
	...
	if (cfg->opt & MONO_OPT_SHARED) {
	...
	} else if (cfg->compile_aot && cfg->cbb->out_of_line && klass->type_token
			&& klass->image == mono_defaults.corlib && !klass->generic_class) {
		...
		if (pass_lw) {
			...
			lw = ((lw + (sizeof(gpointer) - 1)) & ~(sizeof(gpointer) - 1))
					/ sizeof(gpointer);
...
static MonoInst *
handle_box_from_inst(MonoCompile *cfg, MonoInst *val, MonoClass *klass,
		int context_used, MonoInst *data_inst) {
	MonoInst *alloc, *ins;
	...
	if (mono_class_is_nullable(klass)) {
		...
		MonoInst *addr = emit_get_rgctx_method(cfg, context_used, method,
				MONO_RGCTX_INFO_GENERIC_METHOD_CODE);
		MonoInst *rgctx =
				emit_get_rgctx(cfg, cfg->current_method, context_used);
...
handle_castclass(MonoCompile *cfg, MonoClass *klass, MonoInst *src) {
	...
	if (klass->flags & TYPE_ATTRIBUTE_INTERFACE) {
		...
		if (!klass->rank && !cfg->compile_aot && !(cfg->opt & MONO_OPT_SHARED)
		...
		if (klass->rank) {
			...
			if (klass->cast_class == mono_defaults.object_class) {
				...
				mini_emit_class_check_branch(cfg, parent_reg,
						...
						is_null_bb);
				mini_emit_class_check_branch(cfg, eclass_reg,
						mono_defaults.enum_class, OP_PBEQ, is_null_bb);
				MONO_EMIT_NEW_BRANCH_BLOCK (cfg, OP_BR, false_bb);
			} else if (klass->cast_class == mono_defaults.enum_class->parent) {
				mini_emit_class_check_branch(cfg, eclass_reg,
						mono_defaults.enum_class->parent, OP_PBEQ, is_null_bb);
				mini_emit_class_check_branch(cfg, eclass_reg,
						mono_defaults.enum_class, OP_PBEQ, is_null_bb);
				MONO_EMIT_NEW_BRANCH_BLOCK (cfg, OP_BR, false_bb);
			} else if (klass->cast_class == mono_defaults.enum_class) {
				...
				/* the is_null_bb target simply copies the input register to the output */
				mini_emit_isninst_cast(cfg, eclass_reg, klass->cast_class,
						false_bb, is_null_bb);
			}
		} else if (mono_class_is_nullable(klass)) {
			MONO_EMIT_NEW_LOAD_MEMBASE (cfg, klass_reg, vtable_reg, G_STRUCT_OFFSET (MonoVTable, klass));
			/* the is_null_bb target simply copies the input register to the output */
			mini_emit_isninst_cast(cfg, klass_reg, klass->cast_class, false_bb,
					is_null_bb);
		} else {
			if (!cfg->compile_aot && !(cfg->opt & MONO_OPT_SHARED)
					&& (klass->flags & TYPE_ATTRIBUTE_SEALED)) {
				/* the remoting code is broken, access the class for now */
				if (0) {/*FIXME what exactly is broken? This change refers to r39380 from 2005 and mention some remoting fixes were due.*/
					MonoVTable *vt = mono_class_vtable(cfg->domain, klass);
				...
				MONO_EMIT_NEW_LOAD_MEMBASE (cfg, klass_reg, vtable_reg, G_STRUCT_OFFSET (MonoVTable, klass));
				/* the is_null_bb target simply copies the input register to the output */
				mini_emit_isninst_cast(cfg, klass_reg, klass, false_bb,
						is_null_bb);
			}
		}
	}
...
handle_cisinst(MonoCompile *cfg, MonoClass *klass, MonoInst *src) {
	...
	if (klass->flags & TYPE_ATTRIBUTE_INTERFACE) {
		...
		mini_emit_class_check_branch(cfg, klass_reg,
				mono_defaults.transparent_proxy_class, OP_PBNE_UN, false_bb);
		...
		mini_emit_class_check_branch(cfg, klass_reg,
				mono_defaults.transparent_proxy_class, OP_PBNE_UN, no_proxy_bb);
		tmp_reg = alloc_preg(cfg);
...
handle_ccastclass(MonoCompile *cfg, MonoClass *klass, MonoInst *src) {
	...
	if (klass->flags & TYPE_ATTRIBUTE_INTERFACE) {
		...
		MONO_EMIT_NEW_LOAD_MEMBASE (cfg, tmp_reg, obj_reg, G_STRUCT_OFFSET (MonoObject, vtable));
		mini_emit_iface_cast(cfg, tmp_reg, klass, interface_fail_bb,
				ok_result_bb);
		MONO_START_BB (cfg, interface_fail_bb);
		MONO_EMIT_NEW_LOAD_MEMBASE (cfg, klass_reg, tmp_reg, G_STRUCT_OFFSET (MonoVTable, klass));

		mini_emit_class_check(cfg, klass_reg,
 ...
 * Returns NULL and set the cfg exception on error.
 */
static G_GNUC_UNUSED MonoInst * handle_delegate_ctor (MonoCompile *cfg, MonoClass *klass, MonoInst *target, MonoMethod *method) {
	...
	{
		//
		// Im Pre-Patch-Fall wird der Aufruf der Funktion
		// "fireDelegateCtor()", definiert in mono/pre-patch.c,
		// emittiert. So wird das Delegate-Trampoline eines
		// Delegates sofort nach dessen Erzeugung ausgeloest.
		// Hier wird ein optimierter Fall betrachtet, bei dem
		// die Inhalte des Delegate-Konstruktors in den
		// nativen Code eingebettet werden.
		//
		// Die Emittierung des Aufrufs erfolgt auf IR-Ebene,
		// einer linearen Repraesentation des IL-Codes, auf
		// der auch Optimierungen durchgefuehrt werden. Dabei
		// muss zunaechst das Argument der Funktion "fireDelegateCtor()"
		// vorbereitet werden. Das ist in der dieser Phase der
		// JIT-Compilierung eine Anweisung (siehe Makros oben).
		// Die Adresse des Delegate wird mit "get_vreg_to_inst()"
		// bestimmt. Diese Instruktion ist wiederum Argument fuer
		// die Funktion "mono_emit_jit_icall()", welche den
		// eigentlichen Funktionsaufruf emittiert.
		//
		MonoInst * var [1];
		MonoInst * var2 = get_vreg_to_inst(cfg, obj->dreg);
		if (!var2) {
			var2 = mono_compile_create_var_for_vreg (cfg, &klass->byval_arg, OP_LOCAL, obj->dreg);
		} else {
			g_assert (var2->klass == klass);
		}

		var[0] = var2;
		mono_emit_jit_icall (cfg, fireDelegateCtor, var);
	}
	//
	//  Ende Pre-JIT/-Patch Code
	//
...
static void mono_emit_load_got_addr(MonoCompile *cfg) {
...
static gboolean mono_method_check_inlining(MonoCompile *cfg, MonoMethod *method) {
	...
	if (method->is_inflated)
		...
		header = mono_method_get_header(
				((MonoMethodInflated*) method)->declaring);
	else
		header = mono_method_get_header(method);

	if ((method->iflags & METHOD_IMPL_ATTRIBUTE_RUNTIME) || (method->iflags
			& METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) || (method->iflags
			& METHOD_IMPL_ATTRIBUTE_NOINLINING) || (method->iflags
			& METHOD_IMPL_ATTRIBUTE_SYNCHRONIZED) || (method->flags
			& METHOD_ATTRIBUTE_PINVOKE_IMPL) || (method->klass->marshalbyref)
	...
	if (!(cfg->opt & MONO_OPT_SHARED)) {
		...
		if (mono_class_needs_cctor_run(method->klass, NULL)
				&& !((method->klass->flags & TYPE_ATTRIBUTE_BEFORE_FIELD_INIT)))
			return FALSE;
	}
...
static gboolean mini_field_access_needs_cctor_run(MonoCompile *cfg,
		MonoMethod *method, MonoVTable *vtable) {
	...
	if (!(method->flags & METHOD_ATTRIBUTE_STATIC) && (vtable->klass
			== method->klass))
...
static MonoInst*
mini_emit_ldelema_1_ins(MonoCompile *cfg, MonoClass *klass, MonoInst *arr,
		MonoInst *index) {
	MonoInst *ins;
	guint32 size;
...
static MonoInst*
mini_emit_ldelema_2_ins(MonoCompile *cfg, MonoClass *klass, MonoInst *arr,
		MonoInst *index_ins1, MonoInst *index_ins2) {
	int bounds_reg = alloc_preg(cfg);
...
static MonoInst*
mini_emit_ldelema_ins(MonoCompile *cfg, MonoMethod *cmethod, MonoInst **sp,
		unsigned char *ip, gboolean is_set) {
	int rank;
	MonoInst *addr;
	...
	if (rank == 1)
		return mini_emit_ldelema_1_ins(cfg, cmethod->klass->element_class,
				sp[0], sp[1]);

#ifndef MONO_ARCH_EMULATE_MUL_DIV
	/* emit_ldelema_2 depends on OP_LMUL */
	if (rank == 2 && (cfg->opt & MONO_OPT_INTRINS)) {
		return mini_emit_ldelema_2_ins(cfg, cmethod->klass->element_class,
				sp[0], sp[1], sp[2]);
	}
#endif
...
static MonoInst* mini_emit_inst_for_method(MonoCompile *cfg, MonoMethod *cmethod, MonoMethodSignature *fsig, MonoInst **args) {
	...
	if (!runtime_helpers_class) {
		runtime_helpers_class = mono_class_from_name(mono_defaults.corlib, "System.Runtime.CompilerServices", "RuntimeHelpers");
	}
	...
	if (cmethod->klass == mono_defaults.string_class) {
		...
		if (strcmp (cmethod->name, "Enter") == 0) {
			...
			call = (MonoCallInst*)mono_emit_abs_call (cfg, MONO_PATCH_INFO_MONITOR_ENTER, NULL, helper_sig_monitor_enter_exit_trampoline, NULL);
			...
			call = (MonoCallInst*)mono_emit_abs_call (cfg, MONO_PATCH_INFO_MONITOR_EXIT, NULL, helper_sig_monitor_enter_exit_trampoline, NULL);
			mono_call_inst_add_outarg_reg (cfg, call, args [0]->dreg, MONO_ARCH_MONITOR_OBJECT_REG, FALSE);
		...
		if (cfg->method->wrapper_type == MONO_WRAPPER_UNKNOWN && (strcmp (cfg->method->name, "FastMonitorEnter") == 0 || strcmp (cfg->method->name, "FastMonitorExit") == 0)) {
			return NULL;
		}

		if (strcmp (cmethod->name, "Enter") == 0 || strcmp (cmethod->name, "Exit") == 0) {
...
#endif
	} else if (mini_class_is_system_array(cmethod->klass) && strcmp(cmethod->name, "GetGenericValueImpl") == 0) {
		...
		return store;
	} else if (cmethod->klass->image == mono_defaults.corlib && (strcmp(
			cmethod->klass->name_space, "System.Threading") == 0) && (strcmp(
			cmethod->klass->name, "Interlocked") == 0)) {
		ins = NULL;
		...
		if (strcmp(cmethod->name, "Increment") == 0) {
			...
			if (opcode) {
				...
				ins->type = (opcode == OP_ATOMIC_ADD_NEW_I4) ? STACK_I4
						: STACK_I8;
				MONO_ADD_INS (cfg->cbb, ins);
			}
		...
		if ((strcmp(cmethod->name, "CompareExchange") == 0)) {
			...
			else if (sizeof(gpointer) == 8 && fsig->params[1]->type
					== MONO_TYPE_I4)
				size = 8;
			if (size == 4) {
		...
		if (cmethod->name[0] == 'g' && strcmp(cmethod->name,
				"get_IsRunningOnWindows") == 0 && strcmp(cmethod->klass->name,
				"Environment") == 0) {
...
static void mono_save_args(MonoCompile *cfg, MonoMethodSignature *sig,
		MonoInst **sp) {
	MonoInst *store, *temp;
	int i;

	for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
		MonoType *argtype = (sig->hasthis && (i == 0)) ? type_from_stack_type(
				*sp) : sig->params[i - sig->hasthis];
...
static int inline_method(MonoCompile *cfg, MonoMethod *cmethod,
		MonoMethodSignature *fsig, MonoInst **sp, guchar *ip,
		guint real_offset, GList *dont_inline, gboolean inline_allways) {
...
#if (MONO_INLINE_CALLER_LIMITED_METHODS)
	if ((!inline_allways)
			&& !check_inline_caller_method_name_limit(cfg->method))
		return 0;
#endif

	if (cfg->verbose_level > 2)
		printf("INLINE START %p %s -> %s\n", cmethod, mono_method_full_name(
				cfg->method, TRUE), mono_method_full_name(cmethod, TRUE));
	...
	cfg->locals = mono_mempool_alloc0(cfg->mempool, cheader->num_locals
			* sizeof(MonoInst*));
	for (i = 0; i < cheader->num_locals; ++i)
		cfg->locals[i] = mono_compile_create_var(cfg, cheader->locals[i],
				OP_LOCAL);
	...
	costs = mono_method_to_ir(cfg, cmethod, sbblock, ebblock, rvar,
			dont_inline, sp, real_offset, *ip == CEE_CALLVIRT);
	...
	if ((costs >= 0 && costs < 60) || inline_allways) {
		if (cfg->verbose_level > 2)
			printf("INLINE END %s -> %s\n", mono_method_full_name(cfg->method,
					TRUE), mono_method_full_name(cmethod, TRUE));
		...
		if ((prev_cbb->out_count == 1) && (prev_cbb->out_bb[0]->in_count == 1)
				&& (prev_cbb->out_bb[0] != ebblock))
		...
		if ((ebblock->in_count == 1) && ebblock->in_bb[0]->out_count == 1) {
			...
			cfg->cbb = prev;
			if ((prev_cbb->out_count == 1) && (prev_cbb->out_bb[0]->in_count
					== 1) && (prev_cbb->out_bb[0] == prev)) {
...
static gboolean ip_in_bb(MonoCompile *cfg, MonoBasicBlock *bb, const guint8* ip) {
	MonoBasicBlock *b = cfg->cil_offset_to_bb[ip - cfg->cil_start];

	return b == NULL || b == bb;
}

static int get_basic_blocks(MonoCompile *cfg, MonoMethodHeader* header,
		guint real_offset, unsigned char *start, unsigned char *end,
		unsigned char **pos) {
...
mini_get_method_allow_open(MonoMethod *m, guint32 token, MonoClass *klass,
...
static inline MonoMethod *
mini_get_method(MonoCompile *cfg, MonoMethod *m, guint32 token,
		MonoClass *klass, MonoGenericContext *context) {
	MonoMethod *method = mini_get_method_allow_open(m, token, klass, context);

	if (method && cfg && !cfg->generic_sharing_context
			&& mono_class_is_open_constructed_type(&method->klass->byval_arg))
		return NULL;
 ...
 */
static gboolean check_linkdemand(MonoCompile *cfg, MonoMethod *caller,
		MonoMethod *callee) {
	guint32 result;
	...
	if (result == MONO_JIT_LINKDEMAND_ECMA) {
		...
		mono_emit_method_call(cfg, secman->linkdemandsecurityexception, args,
				NULL);
...
static void emit_throw_exception(MonoCompile *cfg, MonoException *ex) {
...
static void ensure_method_is_allowed_to_access_field(MonoCompile *cfg,
		MonoMethod *caller, MonoClassField *field, MonoBasicBlock *bblock,
		unsigned char *ip) {
	/* there's no restriction to access Transparent or SafeCritical fields, so we only check calls to Critical methods */
	if (mono_security_core_clr_class_level(mono_field_get_parent(field))
...
static void ensure_method_is_allowed_to_call_method(MonoCompile *cfg,
		MonoMethod *caller, MonoMethod *callee, MonoBasicBlock *bblock,
		unsigned char *ip) {
	/* there's no restriction to call Transparent or SafeCritical code, so we only check calls to Critical methods */
	if (mono_security_core_clr_method_level(callee, TRUE)
			!= MONO_SECURITY_CORE_CLR_CRITICAL)
		return;
 ...
 */
static const char*
initialize_array_data(MonoMethod *method, gboolean aot, unsigned char *ip,
		MonoClass *klass, guint32 len, int *out_size, guint32 *out_field_token) {
	/*
	 * newarr[System.Int32]
	 * dup
	 ...
	 */
	if (ip[0] == CEE_DUP && ip[1] == CEE_LDTOKEN && ip[5] == 0x4 && ip[6]
			== CEE_CALL) {
		...
		MonoClassField *field = mono_field_from_token(method->klass->image,
				field_token, &dummy_class, NULL);
		int dummy_align;
		...
		*out_field_token = field_token;
		...
		if (!cmethod)
			return NULL;
		if (strcmp(cmethod->name, "InitializeArray") || strcmp(
				cmethod->klass->name, "RuntimeHelpers")
				|| cmethod->klass->image != mono_defaults.corlib)
			return NULL;
		...
		*out_size = size;
		...
		if (!method->klass->image->dynamic) {
			...
			mono_metadata_field_info(method->klass->image, field_index - 1,
					NULL, &rva, NULL);
...
static void set_exception_type_from_invalid_il(MonoCompile *cfg,
		MonoMethod *method, unsigned char *ip) {
	char *method_fname = mono_method_full_name(method, TRUE);
	char *method_code;
...
static void set_exception_object(MonoCompile *cfg, MonoException *exception) {
	cfg->exception_type = MONO_EXCEPTION_OBJECT_SUPPLIED;
	MONO_GC_REGISTER_ROOT (cfg->exception_ptr);
	cfg->exception_ptr = exception;
}

static gboolean generic_class_is_reference_type(MonoCompile *cfg,
		MonoClass *klass) {
	MonoType *type;

	if (cfg->generic_sharing_context)
		type = mini_get_basic_type_from_generic(cfg->generic_sharing_context,
				&klass->byval_arg);
	else
				...
				case OP_NEWARR:
					if (cfg->opt & MONO_OPT_SHARED) {
						...
						MonoVTable *vtable = mono_class_vtable(cfg->domain,
								mono_array_class_get(ins->inst_newa_class, 1));
						...
						dest = mono_emit_jit_icall(cfg,
								mono_array_new_specific, iargs);
						dest->dreg = ins->dreg;
					}
					break;
				case OP_STRLEN:
...
static void emit_stloc_ir(MonoCompile *cfg, MonoInst **sp,
		MonoMethodHeader *header, int n) {
	MonoInst *ins;
	guint32 opcode = mono_type_to_regmove(cfg, header->locals[n]);
	if ((opcode == OP_MOVE) && cfg->cbb->last_ins == sp[0] && ((sp[0]->opcode
			== OP_ICONST) || (sp[0]->opcode == OP_I8CONST))) {
...
emit_optimized_ldloca_ir(MonoCompile *cfg, unsigned char *ip,
	...
	if (ip + 6 < end && (ip[0] == CEE_PREFIX1) && (ip[1] == CEE_INITOBJ)
		...
		token = read32 (ip + 2);
		klass
				= mini_get_class(cfg->current_method, token,
						cfg->generic_context);
	...
	load_error: return NULL;
}

static gboolean is_exception_class(MonoClass *class) {
	while (class) {
 ...
 */
int mono_method_to_ir(MonoCompile *cfg, MonoMethod *method,
	...
	dont_verify = method->klass->image->assembly->corlib_internal ? TRUE
			: FALSE;
	...
	if (!dont_verify
			&& mini_assembly_can_skip_verification(cfg->domain, method)) {
		dont_verify = TRUE;
	...
	cfg->cil_offset_to_bb = mono_mempool_alloc0(cfg->mempool,
			sizeof(MonoBasicBlock*) * header->code_size);
	...
	param_types
			= mono_mempool_alloc(cfg->mempool, sizeof(MonoType*) * num_args);
	if (sig->hasthis)
		param_types[0] = method->klass->valuetype ? &method->klass->this_arg
				: &method->klass->byval_arg;
	...
	if (cfg->method == method) {
		...
		if (cfg->prof_options & MONO_PROFILE_INS_COVERAGE)
			cfg->coverage_info = mono_profiler_coverage_alloc(cfg->method,
					header->code_size);
		...
		for (i = 0; i < header->num_clauses; ++i) {
			...
			if (clause->flags == MONO_EXCEPTION_CLAUSE_FINALLY || clause->flags
					== MONO_EXCEPTION_CLAUSE_FILTER || clause->flags
			...
			if (clause->flags == MONO_EXCEPTION_CLAUSE_NONE || clause->flags
				...
				tblock->in_scount = 1;
				tblock->in_stack = mono_mempool_alloc(cfg->mempool,
						sizeof(MonoInst*));
				tblock->in_stack[0] = mono_create_exvar_for_offset(cfg,
						clause->handler_offset);
			...
			if (clause->flags != MONO_EXCEPTION_CLAUSE_FILTER
					&& clause->data.catch_class && cfg->generic_sharing_context
	...
	pinvoke = (secman && (method->wrapper_type
	...
	if (pinvoke) {
		...
		if (wrapped && (wrapped->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL)) {
			...
			if (custom && mono_custom_attrs_has_attr(custom,
					secman->suppressunmanagedcodesecurity)) {
				pinvoke = FALSE;
			}
			if (custom)
	...
	if ((init_locals || (cfg->method == method && (cfg->opt & MONO_OPT_SHARED)))
			|| cfg->compile_aot || security || pinvoke) {
	...
	if (mono_security_get_mode() == MONO_SECURITY_MODE_CORE_CLR) {
		...
		if (method->wrapper_type == MONO_WRAPPER_MANAGED_TO_NATIVE) {
			MonoMethod *wrapped = mono_marshal_method_from_wrapper(method);
			if (wrapped) {
				gboolean pinvk = (wrapped->flags
						& METHOD_ATTRIBUTE_PINVOKE_IMPL);
				gboolean icall = (wrapped->iflags
				...
				if ((icall || pinvk) && method->klass && method->klass->image) {
					if (!mono_security_core_clr_is_platform_image(
							method->klass->image)) {
						MonoException *ex =
								icall ? mono_get_exception_security()
	...
	for (n = 0; n < header->num_locals; ++n) {
		if (header->locals[n]->type == MONO_TYPE_VOID
				&& !header->locals[n]->byref)
			UNVERIFIED;
	}
	class_inits = NULL;
	...
	stack_start = sp = mono_mempool_alloc0(cfg->mempool, sizeof(MonoInst*)
			* (header->max_stack + 1));
	...
	while (ip < end) {
		...
		if (start_new_bblock) {
			...
			if ((tblock = cfg->cil_offset_to_bb[ip - cfg->cil_start])
					&& (tblock != bblock)) {
				link_bblock(cfg, bblock, tblock);
				if (sp != stack_start) {
					handle_stack_args(cfg, stack_start, sp - stack_start);
		...
		if (cfg->verbose_level > 3)
			printf("converting (in B%d: stack: %d) %s", bblock->block_num,
					(int) (sp - stack_start), mono_disasm_code_one(NULL,
							method, ip, NULL));
		...
		case CEE_STLOC_3: {
			...
			CHECK_LOCAL (n);
			--sp;
			if (!dont_verify_stloc && target_type_is_incompatible(cfg,
					header->locals[n], *sp))
				UNVERIFIED;
		...
		case CEE_DUP: {
			...
			temp = mono_compile_create_var(cfg, type_from_stack_type(ins),
					OP_LOCAL);
		...
		case CEE_JMP: {
			...
			cmethod
					= mini_get_method(cfg, method, token, NULL, generic_context);

			if (!cmethod)
				goto load_error;

			if (cfg->generic_sharing_context && mono_method_check_context_used(
					cmethod))
		...
		case CEE_CALLVIRT: {
			...
			//
			// Wird ein Aufruf einer Interface-Methode emittiert, so ist virtual == 1 -> für Aufzeichnun nutzen
			//
			...
			if (calli) {
				...
				if (method->wrapper_type != MONO_WRAPPER_NONE)
					fsig
							= (MonoMethodSignature *) mono_method_get_wrapper_data(
									method, token);
				else
				...
				if (method->dynamic && fsig->pinvoke) {
					...
					addr = mono_emit_jit_icall(cfg,
							mono_get_native_calli_wrapper, args);
				}
			} else {
				...
				if (!dont_verify && !cfg->skip_visibility) {
					...
					if (method->is_inflated) {
						target_method = mini_get_method_allow_open(method,
								token, NULL,
								&(mono_method_get_generic_container(
										method_definition)->context));
					}
					if (!mono_method_can_access_method(method_definition,
							target_method) && !mono_method_can_access_method(
							method, cil_method))
						METHOD_ACCESS_FAILURE;
				}

				if (mono_security_get_mode() == MONO_SECURITY_MODE_CORE_CLR)
					ensure_method_is_allowed_to_call_method(cfg, method,
							cil_method, bblock, ip);
				...
				if (cmethod->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL
					...
					if (mono_method_signature(cmethod)->pinvoke) {
						MonoMethod *wrapper = mono_marshal_get_native_wrapper(
								cmethod, check_for_pending_exc, FALSE);
						...
						// für iMethod wird dieser Pfad betreten
						//
						fsig = mono_method_get_signature_full(cmethod, image,
								token, generic_context);
					}
				}

				mono_save_token_info(cfg, image, token, cil_method);
				//
				// fsig->param_count enthaelt die Anzahl der Parameter der Interface-Methode
				//
				n = fsig->param_count + fsig->hasthis;
			...
			if (cmethod && ((cmethod->flags & METHOD_ATTRIBUTE_STATIC) || cmethod->klass->valuetype) && (cmethod->klass->generic_class || cmethod->klass->generic_container)) {
				gboolean sharing_enabled = mono_class_generic_sharing_enabled(
						cmethod->klass);
				MonoGenericContext *context = mini_class_get_context(
						cmethod->klass);
				gboolean context_sharable = mono_generic_context_is_sharable(
						context, TRUE);
			...
			if (cfg->generic_sharing_context && cmethod) {
				MonoGenericContext *cmethod_context = mono_method_get_context(
						cmethod);

				context_used = mono_method_check_context_used(cmethod);

				if (context_used && (cmethod->klass->flags
			...
			if (cmethod && virtual && (cmethod->flags & METHOD_ATTRIBUTE_VIRTUAL) && !(MONO_METHOD_IS_FINAL (cmethod) && cmethod->wrapper_type != MONO_WRAPPER_REMOTING_INVOKE_WITH_CHECK) && mono_method_signature(cmethod)->generic_param_count) {
...
#endif
				{
					this_temp = mono_compile_create_var(cfg,
							type_from_stack_type(sp[0]), OP_LOCAL);
					...
					//
					//					if (SNOOP)
					//						printf("method-to-ir() -> mono_emit_calli(): %s",cfg->method->name);
					//
					//  Ende Pre-JIT/-Patch Code
					//
			...
			//
			// für iMethod ist supported_tail_call == 0
			//
#endif
			...
			if ((ins_flag & MONO_INST_TAILCALL)
					&& !cfg->generic_sharing_context && !vtable_arg && cmethod
					&& (*ip == CEE_CALL) && supported_tail_call) {
				MonoCallInst *call;
			...
			/* Inlining */
			if ((cfg->opt & MONO_OPT_INLINE) && cmethod && (!virtual || !(cmethod->flags & METHOD_ATTRIBUTE_VIRTUAL) || MONO_METHOD_IS_FINAL (cmethod))
			...
			if ((cfg->opt & MONO_OPT_TAILC) && *ip == CEE_CALL && cmethod
					== method && ip[5] == CEE_RET && !vtable_arg) {
			...
			if (context_used && !imt_arg && !array_rank
					...
					|| MONO_METHOD_IS_FINAL (cmethod) || !(cmethod->flags
				 ...
				 * indirect call.
				 */
				addr = emit_get_rgctx_method(cfg, context_used, cmethod,
						MONO_RGCTX_INFO_GENERIC_METHOD_CODE);
			}
			...
			if (array_rank) {
				...
				if (strcmp(cmethod->name, "Set") == 0) { /* array Set */
					if (sp[fsig->param_count]->type == STACK_OBJ) {
						...
						mono_emit_jit_icall(cfg, mono_helper_stelem_ref_check,
								iargs);
					}
			...
			ins = mini_redirect_call(cfg, cmethod, fsig, sp, virtual ? sp[0]
					: NULL);
			if (ins) {
				if (!MONO_TYPE_IS_VOID (fsig->ret))
					*sp++ = mono_emit_widen_call_res(cfg, ins, fsig);
			...
			if (vtable_arg) {
				ins = mono_emit_rgctx_method_call_full(cfg, cmethod, fsig, sp,
						virtual ? sp[0] : NULL, NULL, vtable_arg);
			} else if (imt_arg) {
				if (SNOOP) {
					printf(
							"method-to-ir() -> mono_emit_method_call_full(): %s\tvirtual: %i\timt_arg: %p\n",
							cmethod->name, (virtual ? 1 : 0), imt_arg);
				} else {
					;
				}
				ins = (MonoInst*) mono_emit_method_call_full(cfg, cmethod,
						fsig, sp, virtual ? sp[0] : NULL, imt_arg);
			} else {
				//
				// für iMethod wird dieser Pfad betreten
				//
				if (SNOOP) {
					printf(
							"method-to-ir() -> mono_emit_method_call_full(): %s\tvirtual: %i\timt_arg: %p\n",
							cmethod->name, (virtual ? 1 : 0), NULL);
				} else {
					;
				}

				ins = (MonoInst*) mono_emit_method_call_full(cfg, cmethod,
						fsig, sp, virtual ? sp[0] : NULL, NULL);
			}
		...
		case CEE_BRTRUE: {
			MonoInst *cmp;
			gboolean is_short = ((*ip) == CEE_BRFALSE_S) || ((*ip)
					== CEE_BRTRUE_S);
			...
			if (sp[-1]->type == STACK_VTYPE || sp[-1]->type == STACK_R8)
				UNVERIFIED;
			ip++;
			target = ip + opsize + (is_short ? *(signed char*) ip
					: (gint32) read32(ip));
			ip += opsize;
		...
		case CEE_SWITCH: {
			...
			targets = mono_mempool_alloc(cfg->mempool, sizeof(MonoBasicBlock*)
					* n);
			for (i = 0; i < n; ++i) {
			...
			table = mono_mempool_alloc(cfg->mempool,
		...
		case CEE_MUL:
			...
			if ((sp[1]->opcode == OP_ICONST) && mono_arch_is_inst_imm(
					sp[1]->inst_c0)) {
		...
		case CEE_SHR_UN:
			...
			if (((sp[1]->opcode == OP_ICONST) || (sp[1]->opcode == OP_I8CONST))
					&& mono_arch_is_inst_imm(
							sp[1]->opcode == OP_ICONST ? sp[1]->inst_c0
									: sp[1]->inst_l)) {
				int imm_opcode;
		...
		case CEE_CONV_R_UN:
			...
			if ((((*ip) == CEE_CONV_I8) || ((*ip) == CEE_CONV_U8))
					&& (sp[-1]->opcode == OP_ICONST)) {
		...
		case CEE_LDOBJ: {
			...
			if (((ip[5] == CEE_STOBJ) && ip_in_bb(cfg, bblock, ip + 5)
					&& read32 (ip + 6) == token)
			...
			if (method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD) {
				...
				*sp = mono_emit_jit_icall(cfg, mono_string_new_wrapper, iargs);
				...
				if (cfg->opt & MONO_OPT_SHARED) {
					...
					if (cfg->compile_aot) {
						cfg->ldstr_list = g_list_prepend(cfg->ldstr_list,
								GINT_TO_POINTER(n));
					}
					EMIT_NEW_DOMAINCONST (cfg, iargs [0]);
					...
					*sp = mono_emit_jit_icall(cfg, mono_ldstr, iargs);
					...
					if (bblock->out_of_line) {
						...
						if (image == mono_defaults.corlib) {
							...
							*sp = mono_emit_jit_icall(cfg,
									mono_helper_ldstr_mscorlib, iargs);
						} else {
							...
							*sp = mono_emit_jit_icall(cfg, mono_helper_ldstr,
									iargs);
						}
					} else if (cfg->compile_aot) {
						...
						*sp = ins;
						...
						ins->type = STACK_OBJ;
						ins->inst_p0 = mono_ldstr(cfg->domain, image,
								mono_metadata_token_index (n));
						*sp = ins;
						MONO_ADD_INS (bblock, ins);
					}
		...
		case CEE_NEWOBJ: {
			...
			if (cmethod->klass->valuetype
				...
				if (cmethod->is_inflated
						&& mono_method_get_context(cmethod)->method_inst) {
					if (context_used) {
						vtable_arg = emit_get_rgctx_method(cfg, context_used,
								cmethod, MONO_RGCTX_INFO_METHOD_RGCTX);
					} else {
			 ...
			 */
			if (bblock->out_of_line && cmethod->klass->image
					== mono_defaults.corlib && is_exception_class(
					cmethod->klass) && n <= 2 && ((n < 1)
					|| (!fsig->params[0]->byref && fsig->params[0]->type
							== MONO_TYPE_STRING)) && ((n < 2)
					|| (!fsig->params[1]->byref && fsig->params[1]->type
							== MONO_TYPE_STRING))) {
				MonoInst *iargs[3];
				...
				case 0:
					*sp++ = mono_emit_jit_icall(cfg,
							mono_create_corlib_exception_0, iargs);
					break;
				case 1:
					iargs[1] = sp[0];
					*sp++ = mono_emit_jit_icall(cfg,
							mono_create_corlib_exception_1, iargs);
					break;
				case 2:
					iargs[1] = sp[0];
					iargs[2] = sp[1];
					*sp++ = mono_emit_jit_icall(cfg,
							mono_create_corlib_exception_2, iargs);
					break;
				default:
					g_assert_not_reached();
				}
			...
			if (mini_class_is_system_array(cmethod->klass)) {
				...
				if (context_used) {
					*sp = emit_get_rgctx_method(cfg, context_used, cmethod,
				...
				alloc = mono_emit_method_call_full(cfg, cmethod, fsig, sp,
						NULL, NULL);
			} else {
				MonoInst* callvirt_this_arg = NULL;

				if (cmethod->klass->valuetype) {
					iargs[0] = mono_compile_create_var(cfg,
							&cmethod->klass->byval_arg, OP_LOCAL);
					...
					data = emit_get_rgctx_klass(cfg, context_used,
							cmethod->klass, rgctx_info);

					alloc = handle_alloc_from_inst(cfg, cmethod->klass, data,
							FALSE);
					*sp = alloc;
				} else {
					MonoVTable *vtable = mono_class_vtable(cfg->domain,
				...
				if ((cfg->opt & MONO_OPT_INLINE) && cmethod && !context_used
						&& !vtable_arg && mono_method_check_inlining(cfg,
						cmethod) && !mono_class_is_subclass_of(cmethod->klass,
						mono_defaults.exception_class, FALSE) && !g_list_find(
						dont_inline, cmethod)) {
					int costs;

					if ((costs = inline_method(cfg, cmethod, fsig, sp, ip,
							cfg->real_offset, dont_inline, FALSE))) {
						cfg->real_offset += 5;
						bblock = cfg->cbb;
						...
						INLINE_FAILURE;
						mono_emit_method_call_full(cfg, cmethod, fsig, sp,
								callvirt_this_arg, NULL);
					}
				} else if (context_used
						&& (!mono_method_is_generic_sharable_impl(cmethod, TRUE)
								|| !mono_class_generic_sharing_enabled(
					...
					mono_emit_rgctx_calli(cfg, fsig, sp, cmethod_addr,
							vtable_arg);
				} else {
					INLINE_FAILURE;
					ins = mono_emit_rgctx_method_call_full(cfg, cmethod, fsig,
							sp, callvirt_this_arg, NULL, vtable_arg);
				}
			}
		...
		case CEE_CASTCLASS:
			...
			if (context_used) {
				...
				/* klass */
				args[1] = emit_get_rgctx_klass(cfg, context_used, klass,
				...
				*sp++ = ins;
				...
				costs = inline_method(cfg, mono_castclass, mono_method_signature(mono_castclass), iargs, ip, cfg->real_offset, dont_inline, TRUE);
				g_assert(costs > 0);
		...
		case CEE_ISINST: {
			...
			if (context_used) {
				...
				*sp = mono_emit_jit_icall(cfg, mono_object_isinst, args);
				...
				costs = inline_method(cfg, mono_isinst, mono_method_signature(mono_isinst), iargs, ip, cfg->real_offset, dont_inline, TRUE);
				g_assert(costs > 0);
		...
		case CEE_UNBOX_ANY: {
			...
			if (generic_class_is_reference_type(cfg, klass)) {
				...
				if (context_used) {
					...
					iargs[0] = *sp;
					/* klass */
					iargs[1] = emit_get_rgctx_klass(cfg, context_used, klass,
							MONO_RGCTX_INFO_KLASS);
					ins
							= mono_emit_jit_icall(cfg, mono_object_castclass,
									iargs);
					*sp++ = ins;
					ip += 5;
					inline_costs += 2;
				} else if (klass->marshalbyref || klass->flags
						& TYPE_ATTRIBUTE_INTERFACE) {
					MonoMethod *mono_castclass;
					MonoInst *iargs[1];
					int costs;
					...
					costs = inline_method(cfg, mono_castclass,
							mono_method_signature(mono_castclass), iargs, ip,
							cfg->real_offset, dont_inline, TRUE);
		...
		case CEE_BOX: {
			...
			if (!mono_class_is_nullable(klass) && ip + 5 < end && ip_in_bb(cfg,
					bblock, ip + 5) && (ip[5] == CEE_BRTRUE || ip[5]
					== CEE_BRTRUE_S)) {
			...
			if (context_used) {
				...
				if (cfg->opt & MONO_OPT_SHARED)
					...
					rgctx_info = MONO_RGCTX_INFO_VTABLE;
				data = emit_get_rgctx_klass(cfg, context_used, klass,
						rgctx_info);
				*sp++ = handle_box_from_inst(cfg, val, klass, context_used,
						data);
			} else {
		...
		case CEE_STFLD: {
			...
			if (sp[0]->type == STACK_I4 || sp[0]->type == STACK_I8
					|| sp[0]->type == STACK_R8)
				UNVERIFIED;
			...
			foffset = klass->valuetype ? field->offset - sizeof(MonoObject)
					: field->offset;
			if (*ip == CEE_STFLD) {
				if (target_type_is_incompatible(cfg, field->type, sp[1]))
					UNVERIFIED;
				if ((klass->marshalbyref && !MONO_CHECK_THIS (sp [0]))
						|| klass->contextbound || klass
						== mono_defaults.marshalbyrefobject_class) {
					...
					if (cfg->opt & MONO_OPT_INLINE || cfg->compile_aot) {
						costs = inline_method(cfg, stfld_wrapper, mono_method_signature(stfld_wrapper), iargs, ip, cfg->real_offset, dont_inline, TRUE);
						g_assert(costs > 0);
					...
					//--BEGIN RTGC CODE
#ifdef HAVE_RT_GC
					if (field->type->type == MONO_TYPE_CLASS ||field->type->type == MONO_TYPE_ARRAY /*||field->type->type == MONO_TYPE_SZARRAY*/) {
						MonoInst *iargs2[2];
						...
						if (!var)
							var = mono_compile_create_var_for_vreg(cfg,
									&klass->byval_arg, OP_LOCAL, sp[0]->dreg);
						else
							g_assert(var->klass == klass);

						iargs2[0] = var;

						EMIT_NEW_FIELDCONST (cfg, iargs2 [1],field);

						mono_emit_jit_icall(cfg, rtgcStoreValue, iargs2);
						// mono_emit_jit_icall (cfg, mono_get_special_static_data, iargs);
					}
#endif
					//-END RTGC CODE
			...
			if ((klass->marshalbyref && !MONO_CHECK_THIS (sp [0])) || (klass->contextbound) || (klass == mono_defaults.marshalbyrefobject_class)) {
				...
				if (cfg->opt & MONO_OPT_INLINE || cfg->compile_aot) {
					costs = inline_method(cfg, wrapper, mono_method_signature(wrapper), iargs, ip, cfg->real_offset, dont_inline, TRUE);
					bblock = cfg->cbb;
					g_assert(costs > 0);
		...
		case CEE_STSFLD: {
			...
			if ((cfg->opt & MONO_OPT_SHARED) || (cfg->compile_aot && is_special_static) || (context_used && is_special_static)) {
				MonoInst *iargs[2];

				g_assert(field->parent);
				EMIT_NEW_DOMAINCONST (cfg, iargs [0]);
				if (context_used) {
					iargs[1] = emit_get_rgctx_field(cfg, context_used, field, MONO_RGCTX_INFO_CLASS_FIELD);
				} else {
				...
				if (mono_class_needs_cctor_run(klass, method)) {
					...
					vtable = emit_get_rgctx_klass(cfg, context_used, klass,	MONO_RGCTX_INFO_VTABLE);
				 ...
				 */
				static_data = emit_get_rgctx_klass(cfg, context_used, klass,
				...
				EMIT_NEW_DOMAINCONST (cfg, iargs [0]);
				EMIT_NEW_FIELDCONST (cfg, iargs [1], field);
				ins = mono_emit_jit_icall(cfg, mono_class_static_field_address,
						iargs);
			} else {
				MonoVTable *vtable = mono_class_vtable(cfg->domain, klass);
				...
				if (!addr) {
					...
					ins = mono_emit_jit_icall(cfg,
							mono_get_special_static_data, iargs);
				}
			}
			...
			if (*ip == CEE_LDSFLDA) {
				...
				*sp++ = ins;
				...
				if (field->type->type == MONO_TYPE_CLASS ||field->type->type == MONO_TYPE_ARRAY ||field->type->type == MONO_TYPE_SZARRAY) {
					...
					g_assert(field->parent);
					EMIT_NEW_DOMAINCONST (cfg, iargs2 [0]);
					EMIT_NEW_FIELDCONST (cfg, iargs2 [1], field);

					mono_emit_jit_icall(cfg, rtgcStoreStaticValue, iargs2);
					// mono_emit_jit_icall (cfg, mono_get_special_static_data, iargs);
				}
#endif
				//--END RTGC CODE
				...
				if (!context_used && !((cfg->opt & MONO_OPT_SHARED)
						|| cfg->compile_aot) && vtable->initialized
						&& (field->type->attrs & FIELD_ATTRIBUTE_INIT_ONLY)) {
					gpointer addr = (char*) vtable->data + field->offset;
					int ro_type = field->type->type;
					if (ro_type == MONO_TYPE_VALUETYPE
							&& field->type->data.klass->enumtype) {
						ro_type = mono_class_enum_basetype(
								field->type->data.klass)->type;
					}
		...
		case CEE_NEWARR: {
			...
			if (sp[0]->type == STACK_I8 || (SIZEOF_VOID_P == 8 && sp[0]->type
					== STACK_PTR)) {
			...
			*sp++ = ins;
			 ...
			 */
			if ((cfg->opt & MONO_OPT_INTRINS) && ip + 6 < end && ip_in_bb(cfg,
					bblock, ip + 6) && (len_ins->opcode == OP_ICONST)
					&& (data_ptr = initialize_array_data(method,
							cfg->compile_aot, ip, klass, len_ins->inst_c0,
							&data_size, &field_token))) {
		...
		case CEE_LDELEM_REF: {
			...
			if (sp[1]->opcode == OP_ICONST) {
				...
				int offset = (mono_class_array_element_size(klass)
						* sp[1]->inst_c0) + G_STRUCT_OFFSET(MonoArray, vector);
		...
		case CEE_STELEM: {
			...
			if (generic_class_is_reference_type(cfg, klass) && !(sp[2]->opcode == OP_PCONST && sp[2]->inst_p0 == NULL)) {
		...
		case CEE_REFANYVAL: {
			...
			CHECK_OPSIZE (5);
			klass
					= mono_class_get_full(image, read32 (ip + 1),
							generic_context);
			CHECK_TYPELOAD (klass);
			mono_class_init(klass);
			...
			if (!src_var)
				src_var = mono_compile_create_var_for_vreg(cfg,
						&mono_defaults.typed_reference_class->byval_arg,
						OP_LOCAL, sp[0]->dreg);
			...
			if (context_used) {
				...
				klass_ins = emit_get_rgctx_klass(cfg, context_used, klass,
						MONO_RGCTX_INFO_KLASS);
		...
		case CEE_MKREFANY: {
			...
			if (context_used) {
				...
				const_ins = emit_get_rgctx_klass(cfg, context_used, klass,
						MONO_RGCTX_INFO_KLASS);
		...
		case CEE_LDTOKEN: {
			...
			if ((cfg->opt & MONO_OPT_SHARED) && method->wrapper_type != MONO_WRAPPER_DYNAMIC_METHOD && method->wrapper_type != MONO_WRAPPER_SYNCHRONIZED) {
				...
				vtvar = mono_compile_create_var(cfg, &handle_class->byval_arg,
						OP_LOCAL);
				...
				if (method_context_used) {
					...
					ins = mono_emit_jit_icall(cfg,
							mono_ldtoken_wrapper_generic_shared, iargs);
				} else {
				...
				if ((ip + 5 < end) && ip_in_bb(cfg, bblock, ip + 5) && ((ip[5]
						== CEE_CALL) || (ip[5] == CEE_CALLVIRT)) && (cmethod
						= mini_get_method(cfg, method, read32 (ip + 6), NULL,
								generic_context)) && (cmethod->klass
						== mono_defaults.monotype_class->parent) && (strcmp(
						cmethod->name, "GetTypeFromHandle") == 0)) {
					MonoClass *tclass = mono_class_from_mono_type(handle);

					mono_class_init(tclass);
					if (context_used) {
						ins = emit_get_rgctx_klass(cfg, context_used, tclass,
								MONO_RGCTX_INFO_REFLECTION_TYPE);
					} else if (cfg->compile_aot) {
						if (method->wrapper_type) {
							if (mono_class_get(tclass->image,
									tclass->type_token) == tclass
									&& !generic_context) {
					...
					if (context_used) {
						...
						} else if (handle_class
								== mono_defaults.methodhandle_class) {
							ins = emit_get_rgctx_method(cfg, context_used,
									handle, MONO_RGCTX_INFO_METHOD);
						} else if (handle_class
								== mono_defaults.fieldhandle_class) {
		...
		case CEE_LEAVE_S: {
			...
			for (i = 0; i < header->num_clauses; ++i) {
				...
				if (MONO_OFFSET_IN_HANDLER (clause, ip - header->code)
						&& (clause->flags == MONO_EXCEPTION_CLAUSE_NONE) && (ip
						- header->code + ((*ip == CEE_LEAVE) ? 5 : 2))
					...
					exc_ins = mono_emit_jit_icall(cfg,
							mono_thread_get_undeniable_exception, NULL);
			...
			if ((handlers = mono_find_final_block(cfg, ip, target,
		...
		case MONO_CUSTOM_PREFIX: {
			...
			case CEE_MONO_LDPTR: {
				...
				if (cfg->compile_aot && (method->wrapper_type
						== MONO_WRAPPER_MANAGED_TO_NATIVE) && (strstr(
						method->name, "__icall_wrapper_") == method->name)) {
			...
			case CEE_MONO_NEWOBJ: {
				...
				CHECK_OPSIZE (6);
				token = read32 (ip + 2);
				klass = (MonoClass *) mono_method_get_wrapper_data(method,
						token);
				mono_class_init(klass);
			...
			case CEE_MONO_LDNATIVEOBJ:
...

				{
					...
					src = sp[0];
					temp = mono_compile_create_var(cfg, &klass->byval_arg,
							OP_LOCAL);
			...
			case CEE_MONO_DYN_CALL: {
				...
				if (!cfg->dyn_call_var) {
					cfg->dyn_call_var = mono_compile_create_var(cfg,
							&mono_defaults.int_class->byval_arg, OP_LOCAL);
					/* prevent it from being register allocated */
			...
			default:
				g_error("opcode 0x%02x 0x%02x not handled", MONO_CUSTOM_PREFIX,
						ip[1]);
				break;
			}
			break;
		...
		case CEE_PREFIX1: {
			...
			case CEE_ARGLIST: {
				...
				CHECK_STACK_OVF (1);
				vtvar = mono_compile_create_var(cfg,
						&mono_defaults.argumenthandle_class->byval_arg,
						OP_LOCAL);
			...
			case CEE_CLT_UN: {
				...
				type_from_op(cmp, sp[0], sp[1]);
				CHECK_TYPE (cmp);
				if ((sp[0]->type == STACK_I8) || ((SIZEOF_REGISTER == 8)
						&& ((sp[0]->type == STACK_PTR) || (sp[0]->type
								== STACK_OBJ) || (sp[0]->type == STACK_MP))))
					cmp->opcode = OP_LCOMPARE;
			...
			case CEE_LDFTN: {
				...
				n = read32 (ip + 2);
				cmethod
						= mini_get_method(cfg, method, n, NULL, generic_context);
				if (!cmethod)
					goto load_error;
				...
				needs_static_rgctx_invoke
						= mono_method_needs_static_rgctx_invoke(cmethod, TRUE);
				...
				if (mono_security_get_mode() == MONO_SECURITY_MODE_CAS) {
					if (check_linkdemand(cfg, method, cmethod))
						INLINE_FAILURE;
					CHECK_CFG_EXCEPTION;
				} else if (mono_security_get_mode()
				...
				if (!needs_static_rgctx_invoke && !context_used && (sp
						> stack_start) && (ip + 6 + 5 < end) && ip_in_bb(cfg,
						bblock, ip + 6) && (ip[6] == CEE_NEWOBJ)) {
					MonoMethod *ctor_method = mini_get_method(cfg, method,
							read32 (ip + 7), NULL, generic_context);
					if (ctor_method && (ctor_method->klass->parent
						...
						target_ins = sp[-1];
						sp--;
						*sp = handle_delegate_ctor(cfg, ctor_method->klass,
								target_ins, cmethod);
						CHECK_CFG_EXCEPTION;
						ip += 5;
						sp++;
				...
				if (context_used) {
					argconst = emit_get_rgctx_method(cfg, context_used,
							cmethod, MONO_RGCTX_INFO_METHOD);
				} else {
			...
			case CEE_LDVIRTFTN: {
				...
				if (context_used) {
					args[1] = emit_get_rgctx_method(cfg, context_used, cmethod,
							MONO_RGCTX_INFO_METHOD);
					*sp++ = mono_emit_jit_icall(cfg, mono_ldvirtfn_gshared,
							args);
				} else {
					EMIT_NEW_METHODCONST (cfg, args [1], cmethod);
					//
					//  1. emittierter Call auf dem Weg zum Delegate
					//
			...
			case CEE_ENDFILTER: {
				...
				for (cc = 0; cc < header->num_clauses; ++cc) {
					...
					if ((clause->flags & MONO_EXCEPTION_CLAUSE_FILTER) && ((ip
							- header->code) > clause->data.filter_offset && (ip
							- header->code) <= clause->handler_offset)
							&& (!nearest || (clause->data.filter_offset
									< nearest->data.filter_offset))) {
						nearest = clause;
						nearest_num = cc;
					}
				}
			...
			case CEE_CONSTRAINED_:
				...
				if (method->wrapper_type != MONO_WRAPPER_NONE)
					constrained_call
							= (MonoClass *) mono_method_get_wrapper_data(
									method, token);
				else
					constrained_call = mono_class_get_full(image, token,
							generic_context);
			...
			case CEE_INITBLK: {
				...
				if ((ip[1] == CEE_CPBLK) && (cfg->opt & MONO_OPT_INTRINS)
						&& (sp[2]->opcode == OP_ICONST)
						&& ((n = sp[2]->inst_c0) <= sizeof(gpointer) * 5)) {
					mini_emit_memcpy(cfg, sp[0]->dreg, 0, sp[1]->dreg,
							0,
							sp[2]->inst_c0, 0);
				} else if ((ip[1] == CEE_INITBLK) && (cfg->opt
						& MONO_OPT_INTRINS) && (sp[2]->opcode == OP_ICONST)
						&& ((n = sp[2]->inst_c0) <= sizeof(gpointer) * 5)
						&& (sp[1]->opcode == OP_ICONST)
						&& (sp[1]->inst_c0 == 0)) {
					/* emit_memset only works when val == 0 */
					mini_emit_memset(cfg, sp[0]->dreg, 0,
							sp[2]->inst_c0,
							sp[1]->inst_c0, 0);
				} else {
					iargs[0] = sp[0];
			...
			case CEE_SIZEOF: {
				...
				if (mono_metadata_token_table (token) == MONO_TABLE_TYPESPEC) {
					MonoType *type = mono_type_create_from_typespec(image,
							token);
					token = mono_type_size(type, &ialign);
				} else {
					MonoClass *klass = mono_class_get_full(image, token,
							generic_context);
					CHECK_TYPELOAD (klass);
					mono_class_init(klass);
	...
	if (inline_costs < 0) {
		...
		cfg->exception_type = MONO_EXCEPTION_INVALID_PROGRAM;
		cfg->exception_message = g_strdup_printf("Method %s is too complex.",
				mname);
		g_free(mname);
		return -1;
	}
	...
	exception_exit: g_assert(cfg->exception_type != MONO_EXCEPTION_NONE);
	g_slist_free(class_inits);
	dont_inline = g_list_remove(dont_inline, method);
	return -1;

	inline_failure: g_slist_free(class_inits);
	dont_inline = g_list_remove(dont_inline, method);
	return -1;

	load_error: g_slist_free(class_inits);
	dont_inline = g_list_remove(dont_inline, method);
	cfg->exception_type = MONO_EXCEPTION_TYPE_LOAD;
	return -1;

	unverified: g_slist_free(class_inits);
	dont_inline = g_list_remove(dont_inline, method);
	set_exception_type_from_invalid_il(cfg, method, ip);
	return -1;
}

static int store_membase_reg_to_store_membase_imm(int opcode) {
	switch (opcode) {
...
static inline int op_to_op_dest_membase(int store_opcode, int opcode) {
#if defined(TARGET_X86)
	if (!((store_opcode == OP_STORE_MEMBASE_REG) || (store_opcode
...
static inline int op_to_op_store_membase(int store_opcode, int opcode) {
#if defined(TARGET_X86) || defined(TARGET_AMD64)
	switch (opcode) {
	case OP_ICEQ:
...
static inline int op_to_op_src1_membase(int load_opcode, int opcode) {
#ifdef TARGET_X86
	...
	if (!((load_opcode == OP_LOAD_MEMBASE)
			|| (load_opcode == OP_LOADI4_MEMBASE) || (load_opcode
			== OP_LOADU4_MEMBASE)))
		return -1;
...
static inline int op_to_op_src2_membase(int load_opcode, int opcode) {
#ifdef TARGET_X86
	if (!((load_opcode == OP_LOAD_MEMBASE)
			|| (load_opcode == OP_LOADI4_MEMBASE) || (load_opcode
			== OP_LOADU4_MEMBASE)))
...
void mono_handle_global_vregs(MonoCompile *cfg) {
	...
	vreg_to_bb = mono_mempool_alloc0(cfg->mempool, sizeof(gint32*)
			* cfg->next_vreg + 1);
		...
		for (; ins; ins = ins->next) {
			...
			for (regindex = 0; regindex < 4; regindex++) {
				...
				if (regtype == 'l' && !COMPILE_LLVM (cfg)) {
					 ...
					 */
					get_vreg_to_inst (cfg, vreg + 1)->flags
							|= MONO_INST_VOLATILE;
					get_vreg_to_inst (cfg, vreg + 2)->flags
							|= MONO_INST_VOLATILE;
				}
#endif
				...
				if (prev_bb == 0) {
					...
					if (((regtype == 'i' && (vreg < MONO_MAX_IREGS)))
							|| (regtype == 'f' && (vreg < MONO_MAX_FREGS)))
						continue;
					...
					if (!get_vreg_to_inst (cfg, vreg)) {
						...
						case 'v':
							mono_compile_create_var_for_vreg(cfg,
									&ins->klass->byval_arg, OP_LOCAL, vreg);
							break;
						default:
							g_assert_not_reached();
						}
	...
	for (i = 0; i < cfg->num_varinfo; i++) {
		...
		case STACK_VTYPE:
			...
			if ((var->opcode != OP_ARG) && (var != cfg->ret) && !(var->flags
					& (MONO_INST_VOLATILE | MONO_INST_INDIRECT))
					&& (vreg_to_bb[var->dreg] != -1)
					&& (var->klass->byval_arg.type != MONO_TYPE_R4)
					&& !cfg->disable_vreg_to_lvreg) {
				/* 
...
void mono_spill_global_vars(MonoCompile *cfg, gboolean *need_local_opts) {
	...
	*need_local_opts = FALSE;
	...
	vreg_to_lvreg = mono_mempool_alloc0(cfg->mempool, sizeof(guint32)
			* cfg->next_vreg);
	...
	for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
		...
		MONO_BB_FOR_EACH_INS (bb, ins) {
			...
			/***************/
			regtype = spec[MONO_INST_DEST];
			g_assert(((ins->dreg == -1) && (regtype == ' ')) || ((ins->dreg
					!= -1) && (regtype != ' ')));
			prev_dreg = -1;
			...
			if ((ins->dreg != -1) && get_vreg_to_inst (cfg, ins->dreg)) {
				...
				if (var->opcode == OP_REGVAR) {
					ins->dreg = var->dreg;
				} else if ((ins->dreg == ins->sreg1) && (spec[MONO_INST_DEST]
						== 'i') && (spec[MONO_INST_SRC1] == 'i')
						&& !vreg_to_lvreg[ins->dreg] && (op_to_op_dest_membase(
						store_opcode, ins->opcode) != -1)) {
					/*
					...
					if (ins->opcode == OP_MOVE) {
						...
						ins->opcode = op_to_op_dest_membase(store_opcode,
								ins->opcode);
					...
					if (regtype == 'l') {
						...
						if (!lvreg && ((ins->opcode == OP_ICONST)
								|| ((ins->opcode == OP_I8CONST)
										&& (ins->inst_c0 == 0)))) {
							ins->opcode
									= store_membase_reg_to_store_membase_imm(
											store_opcode);
							ins->inst_imm = ins->inst_c0;
							ins->inst_destbasereg = var->inst_basereg;
							ins->inst_offset = var->inst_offset;
							spec = INS_INFO (ins->opcode);
						} else if (!lvreg && ((ins->opcode == OP_MOVE)
								|| (ins->opcode == OP_FMOVE) || (ins->opcode
								== OP_LMOVE))) {
							...
							spec2[MONO_INST_SRC3] = ' ';
							spec = spec2;
						} else if (!lvreg && (op_to_op_store_membase(
								store_opcode, ins->opcode) != -1)) {
							// FIXME: The backends expect the base reg to be in inst_basereg
							ins->opcode = op_to_op_store_membase(store_opcode,
									ins->opcode);
							ins->dreg = -1;
							ins->inst_basereg = var->inst_basereg;
							ins->inst_offset = var->inst_offset;
							 ...
							 * the sregs.
							 */
							if ((!MONO_ARCH_USE_FPSTACK || ((store_opcode
									!= OP_STORER8_MEMBASE_REG) && (store_opcode
			...
			for (srcindex = 0; srcindex < 3; ++srcindex) {
				...
				g_assert(((sreg == -1) && (regtype == ' ')) || ((sreg != -1)
						&& (regtype != ' ')));
				if ((sreg != -1) && get_vreg_to_inst (cfg, sreg)) {
					...
					load_opcode = mono_type_to_load_membase(cfg,
							var->inst_vtype);
					...
					if ((srcindex == 0) && (op_to_op_src1_membase(load_opcode,
							ins->opcode) != -1)) {
						ins->opcode = op_to_op_src1_membase(load_opcode,
								ins->opcode);
						sregs[0] = var->inst_basereg;
						//mono_inst_set_src_registers (ins, sregs);
						ins->inst_offset = var->inst_offset;
					} else if ((srcindex == 1) && (op_to_op_src2_membase(
							load_opcode, ins->opcode) != -1)) {
						ins->opcode = op_to_op_src2_membase(load_opcode,
								ins->opcode);
						...
						if (MONO_IS_REAL_MOVE (ins)) {
							...
							if ((!MONO_ARCH_USE_FPSTACK || ((load_opcode
									!= OP_LOADR8_MEMBASE) && (load_opcode
									!= OP_LOADR4_MEMBASE)))
									&& !((var)->flags & (MONO_INST_VOLATILE
											| MONO_INST_INDIRECT)) && !no_lvreg) {
	...
	for (i = 0; i < cfg->num_varinfo; ++i) {
		...
		if (live_range_start[vreg]) {
			...
			ins->inst_c0 = i;
			ins->inst_c1 = vreg;
			mono_bblock_insert_after_ins(live_range_start_bb[vreg],
					live_range_start[vreg], ins);
		}
		if (live_range_end[vreg]) {
			MONO_INST_NEW (cfg, ins, OP_LIVERANGE_END);
			ins->inst_c0 = i;
			ins->inst_c1 = vreg;
			mono_bblock_insert_after_ins(live_range_end_bb[vreg],
					live_range_end[vreg], ins);
		}
	}
#endif
