...
/*
	if ((SNOOPPATCH)) {
		printf("mono_arch_patch_callsite ()\n");
		fflush(NULL);
	}
*/
	//
...
void mono_arch_patch_plt_entry (guint8 *code, gpointer *got, mgreg_t *regs, guint8 *addr)
{
	...
	//
	if ((SNOOPPATCH)) {
		printf("Mono-RT - mono_arch_patch_plt_entry ()\n");
		fflush(NULL);
...
void mono_arch_nullify_class_init_trampoline (guint8 *code, mgreg_t *regs)
{
	...
	//
	if ((SNOOPPATCH)) {
		printf("Mono-RT - mono_arch_nullify_class_init_trampoline ()\n");
		fflush(NULL);
		printStackTrace(NULL);
	}
	//
...
void mono_arch_nullify_plt_entry (guint8 *code, mgreg_t *regs)
{
	...
	//
	if ((SNOOPPATCH)) {
		printf("Mono-RT - mono_arch_nullify_plt_entry ()\n");
		fflush(NULL);
		printStackTrace(NULL);
	}
	//
...
guchar* mono_arch_create_trampoline_code (MonoTrampolineType tramp_type)
{
	...
	if ((tramp_type == MONO_TRAMPOLINE_RESTORE_STACK_PROT) ||
			(tramp_type == MONO_TRAMPOLINE_AOT_PLT))
	...
	if (!MONO_TRAMPOLINE_TYPE_MUST_RETURN (tramp_type)) {
		...
		//  Beginn Pre-JIT/-Patch Code
		//
		//  Trampoline, die nicht zurueckkehren, bspw. JIT-Trampoline,
		//  muessen fuer das Pre-Patch modifiziert werden, damit sie
		//  zurueckkehren. Dazu wird der Stack-Pointer weiter als
		//  normal erhoeht, so dass zu der Ruecksprungadresse des
		//  aufrufenden Pre-Patch-Codes zurueckgekehrt wird.
		//
		if (PREPTCH && (!PREPTCHCALLVIRT)) {
			//
			//  Das Programm befindet sich im Pre-Patch normaler
			//  Call-Anweisungen.
			//
			if (tramp_type == MONO_TRAMPOLINE_JUMP) {
			...
			if (PREPTCH && PREPTCHCALLVIRT) {
				//
				//  Das Programm befindet sich im Pre-Patch der
				//  VTables und Interface Method Tables. Dabei
				//  werden andere modifizierte Generic Trampoline
				//  benoetigt. Die modifizierten Generic Trampoline
				//  des Pre-Patch normaler Call-Anweisungen werden
				//  vor dem Beginn des VTable-/IMT-Pre-Patch
				//  verworfen.
				//
				x86_alu_reg_imm (buf, X86_ADD, X86_ESP, 10 * 4);
			} else {
				if ((createModifiedDelegateTrampoline_g == 1) && (tramp_type == MONO_TRAMPOLINE_DELEGATE)) {
					//
					//  Dieser Pfad wird waehrend der Initialisierungsphase der
					//  Pre-Patch-Datenstrukturen betreten. Es wird ein modifiziertes
					//  Generic Delegate Trampoline erzeugt, das waehrend des
					//  Delegate-Pre-Patch benoetigt wird. Die globale Variable
					//  "createModifiedDelegateTrampoline_g" wird in der Funktion
					//  "initPrePatchDataStructures()" in pre-patch.c fuer die
					//  Erzeugung eines modifizierten Generic Delegate Trampolines
					//  entsprechend gesetzt. Da dies waehrend der Initialisierungsphase
					//  Monos geschieht, muss wahrscheinlich nicht auf Thread-Sicherheit
					//  geachtet werden.
					//
					x86_alu_reg_imm (buf, X86_ADD, X86_ESP, 10 * 4);
				} else {
		...
		//  Beginn Pre-JIT/-Patch Code
		//
		//  Dieser Pfad wird fuer Trampoline betreten, die
		//  zurueckkehren muessen. Das heiszt, der Programmfluss
		//  setzt nach Ausfuehrung des Trampolines an der Stelle
		//  nach dem Trampoline-Aufruf fort.
		//
		//  Da diese Trampoline waehrend des Pre-Patch aufgerufen
		//  werden, soll der Programmfluss nach ihrer Ausfuehrung
		//  im Pre-Patch-Code fortsetzen. Da die Trampoline per se
		//  an die aufrufende Stelle zurueckkehren muessen diese
		//  Trampoline fuer das Pre-Patch nicht modifiziert werden?!
		//
		if (PREPTCH) {
			x86_alu_reg_imm (buf, X86_ADD, X86_ESP, 11 * 4);
		} else {
	...
	//
	//
	//  Erzeugtes Generic Trampoline in die Liste einfuegen
	//  (nur fuer den Normalfall -- im Pre-Patch-Fall wird die
	//  ModifiedGenericTramp-Liste in main_thread_handler()
	//  in mono/mini/driver.c verwaltet)
	//
//	if (!PREPTCH) {
	insertListElement (GenericTrampoline, (void *) code, (void *) &tramp_type, NULL, 0);
//	}
	//
	//  Ende Pre-JIT/-Patch Code
	//
...
gpointer mono_arch_create_specific_trampoline (gpointer arg1, MonoTrampolineType tramp_type, MonoDomain *domain, guint32 *code_len)
{
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	insertListElement (SpecificTrampoline, (void *) code, (void *) (&tramp_type), (void *) tramp, (unsigned int) (buf - code));
	//
	//  Ende Pre-JIT/-Patch Code
	//
...
//  Untersuchung abgeschlossen
//
gpointer mono_arch_create_generic_class_init_trampoline (void)
{
	guint8 *tramp;
	guint8 *code, *buf;
...
//  Untersuchung abgeschlossen
//
gpointer mono_arch_create_monitor_enter_trampoline (void)
{
	guint32 code_size;
	MonoJumpInfo *ji;
...
//  Untersuchung abgeschlossen
//
gpointer mono_arch_create_monitor_enter_trampoline_full (guint32 *code_size, MonoJumpInfo **ji, gboolean aot)
{
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	//  Durch den Kommandozeilenparameter "--mtsp" wird
	//  das Monitor-Enter-Trampoline gezwungen, stets
	//  den langsamen Pfad ueber das Specific Trampoline,
	//  Generic Trampoline und "mono_monior_enter_trampoline()"
	//  zu nehmen. Dadurch soll der zeitliche Determinismus
	//  gesteigert werden.
	//
	if (MONITORSLOWPATH) {
...
//  Untersuchung abgeschlossen
//
gpointer mono_arch_create_monitor_exit_trampoline (void)
{
	guint32 code_size;
	MonoJumpInfo *ji;
...
//  Untersuchung abgeschlossen
//
gpointer mono_arch_create_monitor_exit_trampoline_full (guint32 *code_size, MonoJumpInfo **ji, gboolean aot)
{
	...
	//
	//  Durch den Kommandozeilenparameter "--mtsp" wird
	//  das Monitor-Exit-Trampoline gezwungen, stets
	//  den langsamen Pfad ueber das Specific Trampoline,
	//  Generic Trampoline und "mono_monior_enter_trampoline()"
	//  zu nehmen. Dadurch soll der zeitliche Determinismus
	//  gesteigert werden.
