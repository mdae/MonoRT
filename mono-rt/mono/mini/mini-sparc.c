...
emit_call (MonoCompile *cfg, guint32 *code, guint32 patch_type, gconstpointer data)
{
	...
	if (0 && v64 && !cfg->compile_aot) {
		...
		target = mono_resolve_patch_target (cfg->method, cfg->domain, NULL, &patch_info, FALSE, 0);
		//
...
//
{
	...
	for (patch_info = ji; patch_info; patch_info = patch_info->next) {
	    ...
	    target = mono_resolve_patch_target (method, domain, code, patch_info, run_cctors, 0);
	    //
	    //  Ende Pre-JIT/-Patch Code
	    //
