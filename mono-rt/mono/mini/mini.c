...
static gpointer mono_jit_compile_method_with_opt (MonoMethod *method, guint32 opt, MonoException **ex, int prePatchAtRuntime);
//
//  Ende Pre-JIT/-Patch Code
//
...
/*
 * @brief:	mono_icall_get_wrapper_full() erzeugt Wraper fuer Internal-JIT-Calls
 *
 * @param	callinfo	Zur Zeit keine Angabe moeglich
 * @param	do_compile	Gibt an, ob der erzeugte Wrapper compiliert wird
 *
 * mono_icall_get_wrapper_full() erzeugt Wrapper fuer den Aufruf
 * von Internal-JIT-Calls. Hat der Parameter "do_compile" den Wert "TRUE",
 * dann wird der erzeugte Wrapper compiliert und die Adresse des nativen
 * Codes des Wrappers zurueckgegeben. Hat der Parameter den Wert "FALSE",
 * so wird lediglich der IL-Code des Wrappers erzeugt, ein JIT-Trampoline
 * auf diesen IL-Code erzeugt und die Adresse des JIT-Trampolines
 * zurueckgegeben.
 */
...
/*
 * @brief	mono_resolve_patch_target() gibt die Adresse des
 * Codes zur Behandlung eines Patches zurueck
 * @param	method		Zeiger auf die MonoMethod-Struktur der Methode, did die Patches enthaelt
 * @param	domain		Zeiger auf die Application Domain, innerhalb der die Methode
 * compiliert wurde
 * @param	code		Endgueltige absolute Start-Adresse des nativen Codes der Methode
 * @param	patch_info	Zeiger auf den in diesem Aufruf zu bearbeitenden Patch
 * @param	run_cctors	Gibt an, ob es sich um eine AOT-Compilierung handelt
 * @param	prePatchAtRuntime	Gibt an, ob ein emittierter Patch sofort gepatcht werden soll
 *
 */
gpointer
//
...
mono_resolve_patch_target (MonoMethod *method, MonoDomain *domain, guint8 *code, MonoJumpInfo *patch_info, gboolean run_cctors, int prePatchAtRuntime)
//
//  Ende Pre-JIT/-Patch Code
//
{
	...
	case MONO_PATCH_INFO_INTERNAL_METHOD: {
		...
		//  Beginn Pre-JIT/-Patch Code
		//
		insertListElement(CallCodeInstruction, (void *) ip, (void *) method, (void *) target, (unsigned int) patch_info->type);
		//
		//  Ende Pre-JIT/-Patch Code
		//
		break;
	...
	case MONO_PATCH_INFO_VTABLE:
		//
		//  Beginn Pre-JIT/-Patch CodeNEU
		//
		// Original:
		//target = mono_class_vtable (domain, patch_info->data.klass);
		// Aenderung:
		target = mono_class_vtable_full (domain, patch_info->data.klass, TRUE);
		//
		//  Ende Pre-JIT/-Patch CodeNEU
		//
		g_assert (target);
		break;
	case MONO_PATCH_INFO_CLASS_INIT: {
...
//			MonoMethod * wrapperMethod;
			...
			//  Beginn Pre-JIT/-Patch Code
			//
			//  Falls eine synchrone Methode angsprungen werden soll, so
			//  wird das Patch-Target auf den Wrapper der Methode gelegt.
			//

			if ((patch_info->data.method->iflags & METHOD_IMPL_ATTRIBUTE_SYNCHRONIZED) != 0) {

				if (patch_info->data.method->wrapper_type != MONO_WRAPPER_SYNCHRONIZED) {
					wrapperMethod = mono_marshal_get_synchronized_wrapper(patch_info->data.method);
					patch_info->data.method = wrapperMethod;
				}
			}
...
void
mono_codegen (MonoCompile *cfg, int prePatchAtRuntime)
{
	MonoBasicBlock *bb;
	int max_epilog_size;
	guint8 *code;
...
#endif
	//
	//  In "mono_arch_patch_code()" werden Call- oder Jump-Anweisungen
	//  (Patches) der gerade compilierten Methode behandelt. Der native
	//  Code der Methode wurde bereits an seine endgueltige Position
	//  verschoben.
	//
	//  Die Patches werden darin aufgezeichnet und fuer das Pre-Patch
	//  verwendet.
	//
	mono_arch_patch_code (cfg->method, cfg->domain, cfg->native_code, cfg->patch_info, cfg->run_cctors, prePatchAtRuntime);
	//
	//  Beginn Pre-JIT/-Patch Code
	//
	//  Erfassung der gerade compilierten Methode fuer ein eventuelles
	//  spaetere Pre-Patch dieser Patches.
	//
	insertListElement (NativeCode, (void *) cfg, NULL, NULL, 0);
	//
	//  Ende Pre-JIT/-Patch Code
	//
	if (cfg->method->dynamic) {
...
mini_method_compile (MonoMethod *method, guint32 opts, MonoDomain *domain, gboolean run_cctors, gboolean compile_aot, int parts, int prePatchAtRuntime)
//
//  Ende Pre-JIT/-Patch Code
//
{
	...
	if ((SNOOPJIT)) {
		ret = asprintf(&messageToUser,"Mono-RT - mini_method_compile ()\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
...
#define DEBUGSSA_CLASS "Tests"
	...
	if (COMPILE_LLVM (cfg)) {
		...
		//
		//  Hier startet die Generierung des nativen Maschinencodes aus der
		//  der IR des CIL-Codes. Optimierungen fanden bereits statt.
		//
		mono_codegen (cfg, prePatchAtRuntime);
	}
	...
	if (SNOOPASM) {
		...
		for (i = 0; i < MAXMETHODSASMTRACE; i++) {
			...
			if (acAsmMethods_g[i] != NULL)
			{
				if (strncmp(acAsmMethods_g[i], cfg->method->name, strlen(cfg->method->name)) == 0)
				{
...
mono_jit_compile_method_inner (MonoMethod *method, MonoDomain *target_domain, int opt, MonoException **jit_ex, int prePatchAtRuntime)
//
//  Ende Pre-JIT/-Patch Code
//
{
	...
	if (opt & MONO_OPT_AOT) {
		...
		if ((code = mono_aot_get_method (domain, method))) {
			...
			// Aenderung:
			vtable = mono_class_vtable_full (domain, method->klass, TRUE);
			//
			//  Ende Pre-JIT/-Patch CodeNEU
			//
			g_assert (vtable);
			...
			//  Beginn Pre-JIT/-Patch CodeNEU
			//
			insertListElement (NativeCode, NULL, (void *) code, (void *) method, 0);
			//
			//  Ende Pre-JIT/-Patch CodeNEU
			//
			return code;
	...
	cfg = mini_method_compile (method, opt, target_domain, TRUE, FALSE, 0, prePatchAtRuntime);
	//
	//  Ende Pre-JIT/-Patch Code
	//
...
/*
	if (domain_jit_info (target_domain)->jump_target_hash) {
		...
		for (tmp = list; tmp; tmp = tmp->next)
			mono_arch_patch_code (NULL, target_domain, tmp->data, &patch_info, TRUE, 0);
		g_slist_free (list);
	}
*/
	//
...
mono_jit_compile_method_with_opt (MonoMethod *method, guint32 opt, MonoException **ex, int prePatchAtRuntime)
//
//  Ende Pre-JIT/-Patch Code
//
{
	...
	code = mono_jit_compile_method_inner (method, target_domain, opt, ex, prePatchAtRuntime);
	//
	//  Ende Pre-JIT/-Patch Code
	//
	if (!code)
...
mono_jit_compile_method (MonoMethod *method, int prePatchAtRuntime)
//
//  Ende Pre-JIT/-Patch Code
//
{
	...
	code = mono_jit_compile_method_with_opt (method, default_opt, &ex, prePatchAtRuntime);
	//
	//  Ende Pre-JIT/-Patch Code
	//
	if (!code) {
...
mono_jit_runtime_invoke (MonoMethod *method, void *obj, void **params, MonoObject **exc)
{
	...
	if (!info) {
		...
		if (method->klass->rank && (method->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) &&
			...
			info->compiled_method = mono_jit_compile_method_with_opt (method, default_opt, &jit_ex, 0);
			//
			//  Ende Pre-JIT/-Patch Code
			//
		...
		if (!info->dyn_call_info) {
			...
			info->runtime_invoke = mono_jit_compile_method (invoke, 0);
			//
			//  Ende Pre-JIT/-Patch Code
			//
		}
	...
	if (info->dyn_call_info) {
		...
		if (!dyn_runtime_invoke) {
			...
			dyn_runtime_invoke = mono_jit_compile_method (invoke, 0);
			//
			//  Ende Pre-JIT/-Patch Code
			//
		}
...
mono_jit_create_remoting_trampoline (MonoDomain *domain, MonoMethod *method, MonoRemotingTarget target)
{
	...
	if ((method->flags & METHOD_ATTRIBUTE_VIRTUAL) && mono_method_signature (method)->generic_param_count) {
		return mono_arch_create_specific_trampoline (method, MONO_TRAMPOLINE_GENERIC_VIRTUAL_REMOTING, domain, NULL);
	}

	if ((method->flags & METHOD_ATTRIBUTE_ABSTRACT) || (mono_method_signature (method)->hasthis && (method->klass->marshalbyref || method->klass == mono_defaults.object_class))) {
...
mini_init (const char *filename, const char *runtime_version)
{
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	// Die Funktion "fireDelegateCtor" zum Pre-Patch eines
	// Delegate wird durch diese Initialisierung fuer Module
	// im Verzeichnis "mono/metadata" verfuegbar gemacht.
	//
	callbacks.fireDelegateCtor = fireDelegateCtor;
	callbacks.insertListElement = insertListElement;
	callbacks.mono_jit_compile_method = mono_jit_compile_method;
	callbacks.printStackTrace = printStackTrace;
	//
	//  Ende Pre-JIT/-Patch Code
	//
...
#define JIT_INVOKE_WORKS
	...
	//  Beginn Pre-JIT/-Patch-Code
	//
	// Die mittels "mono_add_internal_call()" registrierten C-Funktionen
	// koennen aus einem C#-Programm heraus aufgerufen werden. Diese
	// Internal Calls werden zum Pre-JIT verwendet. Um Exceptions waehrend
	// des Pre-JIT (und zukuenftig auch waehrend des Pre-Patch) abzufangen,
	// ruft ein C#-Programm diese Mono-internen C-Funktionen auf. Dabei
	// muessen die Aufrufe der Mono-internen C-Funktionen auf C#-Seite
	// genau benannt sein, d.h., mit Namespace, Klasse und Name.
	//
	mono_add_internal_call ("PreJitClass::preCompile", preCompile);
	mono_add_internal_call ("PreJitClass::insertInPreJitExclusionList", insertInPreJitExclusionList);
	mono_add_internal_call ("PrePatchClass::prepatchCode", prepatchCode);
	mono_add_internal_call ("MonitoringClass::setMonitoringOptionsInternal", setMonitoringOptionsInternal);
	//
	//  Ende Pre-JIT/-Patch Code
	//
...
#define JIT_CALLS_WORK
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	// Registrierung der Funktion "fireDelegateCtor()" aus "mono/mini/pre-patch.c",
	// so dass ein Aufruf dieser Funktion vom JIT-Compiler in den nativen Code
	// emittiert werden kann. Die Funktion "fireDelegateCtor()" wird zum
	// Pre-Patch eines (neu erzeugten) Delegate aufgerufen. Ihr Aufruf wird
	// im optimierten Fall einer Delegate-Erzeugung vom JIT-Compiler in den
	// nativen Code emittiert (siehe "handle_delegate_ctor()" in
	// "mono/mini/method-to-ir.c").
	//
	register_icall (fireDelegateCtor, "fireDelegateCtor", "void ptr", FALSE);
	//
	//  Ende Pre-JIT/-Patch Code
	//
#ifdef HAVE_RT_GC
	register_icall (rtgcStoreStaticValue,"rtgcStoreStaticValue","void ptr ptr",FALSE);
	register_icall (rtgcStoreValue,"rtgcStoreValue","void ptr ptr",FALSE);
#endif
...
}//	struct MonoNativeCodeList *NativeCodeListPtr;
