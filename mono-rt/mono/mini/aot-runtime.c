	...
	if (n_patches) {
		...
		for (pindex = 0; pindex < n_patches; ++pindex) {
			...
			if (!amodule->got [got_slots [pindex]]) {
				...
				amodule->got [got_slots [pindex]] = mono_resolve_patch_target (method, domain, code, ji, TRUE, 0);
				//
				//  Ende Pre-JIT/-Patch Code
				//
...
mono_aot_get_method (MonoDomain *domain, MonoMethod *method)
{
	...
	//
	insertListElement(AotModule, (void *) amodule, NULL, NULL, 0);
	//
...
mono_aot_plt_resolve (gpointer aot_module, guint32 plt_info_offset, guint8 *code)
{
	...
	if (mono_aot_only && ji.type == MONO_PATCH_INFO_METHOD && !ji.data.method->is_generic && !mono_method_check_context_used (ji.data.method) && !(ji.data.method->iflags & METHOD_IMPL_ATTRIBUTE_SYNCHRONIZED) &&
		...
		target = mono_jit_compile_method (ji.data.method, 0);
		//
		//  Ende Pre-JIT/-Patch Code
		//
	} else {
		...
		target = mono_resolve_patch_target (NULL, mono_domain_get (), NULL, &ji, TRUE, 0);
		//
		//  Ende Pre-JIT/-Patch Code
		//
	}
	...
	if (n_patches) {
		...
		for (pindex = 0; pindex < n_patches; ++pindex) {
			...
			if (ji->type == MONO_PATCH_INFO_JIT_ICALL_ADDR) {
				...
				target = mono_resolve_patch_target (NULL, NULL, code, ji, FALSE, 0);
				//
				//  Ende Pre-JIT/-Patch Code
				//
				g_assert (target);
