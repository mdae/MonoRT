...
/*
 * @brief	mono_arch_patch_code() initialisiert die Patches (Spruenge, Calls)
 * einer JIT-compilierten Methode
 * @param	method		Zeiger auf die MonoMethod-Struktur der compilierten Methode
 ...
 * @param	ji			Verkettete Liste der einzelnen Patches innerhalb der Methode
...
mono_arch_patch_code (MonoMethod *method, MonoDomain *domain, guint8 *code, MonoJumpInfo *ji, gboolean run_cctors, int prePatchAtRuntime)
//
//  Ende Pre-JIT/-Patch Code
//
{
	...
	for (patch_info = ji; patch_info; patch_info = patch_info->next) {
	    ...
	    target = mono_resolve_patch_target (method, domain, code, patch_info, run_cctors, prePatchAtRuntime);
	    //
	    //  Ende Pre-JIT/-Patch Code
	    //
		if (compile_aot) {
