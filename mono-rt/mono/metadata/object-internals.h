...
//
//  Die Enumeration fuer das Einfuegen der fuer das
//  Pre-Patch benoetigten Datenstrukturen erfolgt hier,
//  da "pre.h" nicht ohne Fehler eingebunden werden
//  kann. Die Header-Datei "object-internals.h" wird
//  in "mono/mini/pre.h" eingebunden, so dass die
//  Enumeration dort bekannt ist.
//
enum TrampolineType {
	SpecificTrampoline,
	GenericTrampoline,
	ModifiedGenericTrampoline,
	NativeCode,
	CallCodeInstruction,
	Class,
	VTable,
...
typedef struct {
	...
	int		(*insertListElement) (enum TrampolineType, void *, void *, void *, unsigned int);
	gpointer (*mono_jit_compile_method) (MonoMethod *, int);
