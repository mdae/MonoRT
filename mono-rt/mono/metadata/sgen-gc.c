 ...
 */
static MonoMethod* create_allocator (int atype)
{
	int p_var, size_var;
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	if ((SNOOPWRAP) || (snoopwrapper_org == 1)) {
		printf("Mono-RT - create_allocator() (SGEN):\tThread: %lu\n", pthread_self());
		fflush(NULL);
	}
	//
	//
...
MonoMethod* mono_gc_get_write_barrier (void)
{
	...
	if (snoopwrapper_org == 1) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_gc_get_write_barrier () (MONO_WRAPPER_WRITE_BARRIER, Cache-Miss):\n\tThread: %lu\n\tAppDomain: %u\n",
