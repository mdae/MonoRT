...
#ifndef DISABLE_REFLECTION_EMIT
static MonoMethod* reflection_methodbuilder_to_mono_method (MonoClass *klass, ReflectionMethodBuilder *rmb, MonoMethodSignature *sig) {
	MonoError error;
	MonoMethod *m;
	MonoMethodNormal *pm;
	...
	if (m->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) {
		...
		return m;
	} else if (!(m->flags & METHOD_ATTRIBUTE_ABSTRACT) && !(m->iflags & METHOD_IMPL_ATTRIBUTE_RUNTIME)) {
		MonoMethodHeader *header;
		guint32 code_size;
		gint32 max_stack, i;
		gint32 num_locals = 0;
	...
	if (rmb->refs) {
		...
		if (snoopwrapper_org == 1) {
			ret = asprintf(&messageToUser,"Mono-RT - reflection_methodbuilder_to_mono_method () (MONO_WRAPPER_DYNAMIC_METHOD):\n\tThread: %lu\n\tAppDomain: %u\n",
