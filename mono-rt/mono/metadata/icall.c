...
//
/**
 * @brief ves_icall_System_Object_combinedDelegate() ruft ein Delegate-Trampoline auf.
 *
 * @param this	Zeiger auf eine Delegate-Struktur
 *
 * Die Funktion "ves_icall_System_Object_combinedDelegate()" ist
 * die Implementierung des Internal Calls "combinedDelegate()".
 * Die Registrierung als Implememtierung des Internal Calls
 * befindet sich in "mono/metadata/icall-def.h". Der Internal Call
 * "combinedDelegate()" wird aus C#-Code heraus aufgerufen und
 * erfolgt in den folgenden mscorlib-Methoden:
 *
 * 	"Delegate Combine (Delegate)" in "mcs/class/corlib/System/Delegate.cs"
 *	"Delegate Combine (Delegate[])" in "mcs/class/corlib/System/Delegate.cs"
 *	"Delegate Revome (Delegate, Delegate)" in "mcs/class/corlib/System/Delegate.cs"
 *	"Delegate CombineImpl (Delegate) in "mcs/class/corlib/System/MutlicastDelegate.cs"
 *
 * Er dient dem Zweck des Delegate-Pre-Patch nach einer
 * Verknuepfung mehrere Delegates, da dabei i.d.R. eine
 * neue Delegate-Instanz erzeugt wird, die noch nicht
 * gepatcht ist.
 */
static void
ves_icall_System_Object_combinedDelegate (MonoObject *this)
{

#if defined(TARGET_X86) // Archtitekturabhaengiger Pre-Patch-Code
	//
	// Dieses Makro findet man in vielen Funktionen. Daher
	// wird es auch hier eingefuegt.
	//
	MONO_ARCH_SAVE_REGS;
	...
	{
		//
		// Aufruf der Funktion "fireDelegateCtor()", die in "mono/mini/pre-patch.c"
		// definiert ist. Sie loest einen Pre-Patch des Delegate-Trampolines
		// aus. Der Aufruf selbst erfolgt ueber ein Callback, welches
		// in der Datenstruktur "MonoRuntimeCallbacks" (mono/metadata/object-internals.h)
		// definiert ist. Die Initialisierung der Datenstruktur erfolgt
		// in "mini_init()" in "mono/mini/mini.c".
		//
		mono_get_runtime_callbacks ()->fireDelegateCtor (delegate);
	}
	return;
