...
mono_compile_method (MonoMethod *method)
{
	...
	return default_mono_compile_method (method, 0);
	//
	//  Ende Pre-JIT/-Patch Code
	//
}
...
mono_install_imt_trampoline (gpointer tramp_code)
{
	...
	//  Beginn Pre-JIT/-Patch-Code
	//
	//  Globale Variable zur Speicherung der Adresse
	//  des Specific IMT Trampolines. Diese Variable
	//  ist in mono/mini/pre.h definiert.
	//
	pImtTramp_g = imt_trampoline;
	//
	//  Ende Pre-JIT/-Patch-Code
	//
}
...
mono_install_vtable_trampoline (gpointer tramp_code)
{
	...
	//
	//  Globale Variable zur Speicherung der Adresse
	//  des Specific VTable Trampolines. Diese Variable
	//  ist in mono/mini/pre.h definiert.
	//
	pVTableTramp_g = vtable_trampoline;
	//
	//  Ende Pre-JIT/-Patch-Code
	//
}
...
mono_class_vtable (MonoDomain *domain, MonoClass *class)
{
	...
	// Aenderung:
	//return mono_class_vtable_full (domain, class, TRUE);
	//
...
mono_class_create_runtime_vtable (MonoDomain *domain, MonoClass *class, gboolean raise_on_error)
{
	...
	//
	if ((SNOOPPATCH)) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_class_create_runtime_vtable ()\n\tKlasse: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				class->name, pthread_self(), mono_domain_get()->domain_id);
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	//  Verwalten der initialisierten VTables in einer Hashtabelle
	//
	callbacks.insertListElement (VTable, (void *) vt, (void *) imt_trampoline, (void *) vtable_trampoline, (unsigned int) interface_offsets);
	//
	//  Ende Pre-JIT/-Patch Code
	//
	return vt;
...
mono_delegate_ctor_with_method (MonoObject *this, MonoObject *target, gpointer addr, MonoMethod *method)
{
	...
	{
		//
		// Ausloesen des Delegate-Pre-Patch fuer das neu
		// erzeugte Delegate. Dazu wird die Funktion "fireDelegateCtor()"
		// aus der Datei "mono/mini/pre-patch.c" aufgerufen. Diese
		// Funktion wird ebenfalls zum Delegate-Pre-Patch im
		// optimierten Fall (Teile des Delegate-Konstruktors werden
		// in den nativen Code eingebettet; i.d.R. bei CIL-Folge
		// LDFTN + NEWOBJ; siehe "handle_delegate_ctor()" in
		// "mono/mini/method-to-ir.c"). Im optimierten Fall wird
		// deren Aufruf in den nativen Code emittiert. Hier im
		// unoptimierten Fall wird sie im Delegate-Konstruktor ueber
		// ein Mono-internes Callback aufgerufen, da das Build-System
		// Funktionsaufrufe ueber Verzeichnisgrenzen hinweg kompliziert
		// macht.
		//
		// "callbacks" ist eine Datenstruktur vom Typ "MonoRuntimeCallbacks",
		// siehe "mono/metadata/object-internals.h". Sie wird in der Funktion
		// "mini_init()" in "mono/mini/mini.c" initialisiert.
		//
		callbacks.fireDelegateCtor(delegate);
	}
	//
	//  Ende Pre-JIT/-Patch Code
	//
