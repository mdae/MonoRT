...
cominterop_get_native_wrapper_adjusted (MonoMethod *method)
{
	...
	//
	if ((SNOOPWRAP)) {
		ret = asprintf(&messageToUser,"Mono-RT - cominterop_get_native_wrapper_adjusted () (MONO_WRAPPER_MANAGED_TO_NATIVED):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
...
mono_cominterop_get_native_wrapper (MonoMethod *method)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_cominterop_get_native_wrapper () (MONO_WRAPPER_COMINTEROP, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_cominterop_get_invoke (MonoMethod *method)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_cominterop_get_invoke () (MONO_WRAPPER_COMINTEROP_INVOKE, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
	...
	if (!ccw_entry) {
		...
		for (i = iface->method.count-1; i >= 0;i--) {
			...
			if (snoopwrapper_org == 1) {
				ret = asprintf(&messageToUser,"Mono-RT - cominterop_get_ccw() (MONO_WRAPPER_NATIVE_TO_MANAGED):\n\tThread: %lu\n\tAppDomain: %u\n",
...
cominterop_get_managed_wrapper_adjusted (MonoMethod *method)
{
	...
	//
	if (snoopwrapper_org == 1) {
		ret = asprintf(&messageToUser,"Mono-RT - cominterop_get_managed_wrapper_adjusted () (MONO_WRAPPER_COMINTEROP):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
