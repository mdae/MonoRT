...
mono_class_init (MonoClass *class)
{
	...
	if ((SNOOPPATCH))
	{
		ret = asprintf(&messageToUser,"Mono-RT - mono_class_init ()\n\tKlasse: %s\n\tThread: %lu\n\tAppDomain: %u\n",
 ...
 leave:
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	if ((mono_get_runtime_callbacks ()->insertListElement) != NULL) {
		mono_get_runtime_callbacks ()->insertListElement(Class, (void *) class, NULL, NULL, 0);
	}
	//
	//  Ende Pre-JIT/-Patch Code
	//
