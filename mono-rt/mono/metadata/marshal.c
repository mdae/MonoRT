...
//
#include "mono/mini/controlParameter.h"
//
...
mono_delegate_to_ftnptr (MonoDelegate *delegate)
{
	...
	//
	//  Das Pre-Patch des erzeugten Wrappers wird nur dann
	//  aktiviert, wenn das Pre-Patch der Patches und der
	//  VTables abgeschlossen wurde (doPrePatch_g == 0) und wenn
	//  Delegate-Pre-Patch aktiviert wurde.
	//
	//  Die Verwendung der Funktion "mono_jit_compile_method()"
	//  anstatt von "mono_compile_method()" erfolgte, da der
	//  Funktion "mono_compile_method()" ohne groszen Aufwand
	//  kein zusaetzlicher Parameter uebergeben werden kann.
	//
	prePatchAtRuntime = (((doPrePatch_g == 0) && (((doPrePatchCmd_g & PREPATCHDEL) != 0) || ((doPrePatchCmd_g & PREPATCHDELEXT) != 0))) ? 1 : 0);
	delegate->delegate_trampoline = mono_get_runtime_callbacks ()->mono_jit_compile_method (wrapper, prePatchAtRuntime);
	//
	//  Ende Pre-JIT/-Patch Code
	//
...
mono_marshal_get_remoting_invoke (MonoMethod *method)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_remoting_invoke () (MONO_WRAPPER_REMOTING_INVOKE, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
		...
		mono_get_runtime_callbacks ()->printStackTrace(&messageToUser);
	}
	//
	//  Ende Pre-JIT/-Patch Code
	//
...
mono_marshal_get_xappdomain_dispatch (MonoMethod *method, int *marshal_types, int complex_count, int complex_out_count, int ret_marshal_type)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_xappdomain_dispatch () (MONO_WRAPPER_XDOMAIN_DISPATCH, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_xappdomain_invoke (MonoMethod *method)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_xappdomain_invoke () (MONO_WRAPPER_XDOMAIN_INVOKE, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_remoting_invoke_with_check (MonoMethod *method)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_remoting_invoke_with_check () (MONO_WRAPPER_REMOTING_INVOKE_WITH_CHECK, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
{
	MonoMethodSignature *sig, *csig, *callsig, *tmp_sig;
...
#endif
	//
	//  Der folgende Code wurde aus marshal.c von Mono-2.8.2 übernommen
	//
#if 0
...
#endif
	//
	//  Der folgende Code entspricht dem originial Mono-2.6.1-Code
	//
	target_klass = mono_defaults.object_class;
#if 0
	/*
	...
	if (need_direct_wrapper) {
		...
		//
		//  Durch diese Deaktivierung muss für jede Methode der
		//  Wrapper erzeut angelegt werden, da nicht mehr nach
		//  Call-Signatur gecached wird.
		//
		cache = get_cache (&target_klass->image->runtime_invoke_cache, (GHashFunc)mono_signature_hash, (GCompareFunc)runtime_invoke_signature_equal);
...
#if 0		/* Make a copy of the signature from the image mempool */
		tmp_sig = callsig;
	...
	//
	if ((SNOOPWRAP)) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_runtime_invoke() (Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_runtime_invoke_dynamic (void)
{
	...
	if (method) {
		...
		//
		if (SNOOPWRAP) {
			printf("Mono-RT - mono_marshal_get_runtime_invoke_dynamic() (Methode vorhanden):\tMethode: %s\tThread: %lu\n", mono_method_full_name(method, TRUE), pthread_self());
			fflush(NULL);
		}
		//
		//  Ende Pre-JIT/-Patch Code
	...
	//
	if (SNOOPWRAP) {
		printf("Mono-RT - mono_marshal_get_runtime_invoke_dynamic() (Methode nicht vorhanden):\tMethode: %s\tThread: %lu\n", mono_method_full_name(method, TRUE), pthread_self());
		fflush(NULL);
	}
	//
	//  Ende Pre-JIT/-Patch Code
	...
	//
	if (SNOOPWRAP) {
		printf("Mono-RT - mono_marshal_get_runtime_invoke_dynamic():\tMethode: %s\tThread: %lu\n", mono_method_full_name(method, TRUE), pthread_self());
		fflush(NULL);
	}
	//
	//  Ende Pre-JIT/-Patch Code
...
mono_marshal_get_ldfld_remote_wrapper (MonoClass *klass)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_ldfld_remote_wrapper () (MONO_WRAPPER_LDFLD_REMOTE, Cache-Miss):\n\tThread: %lu\n\tAppDomain: %u\n",
...
mono_marshal_get_ldfld_wrapper (MonoType *type)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_ldfld_wrapper () (MONO_WRAPPER_LDFLD, Cache-Miss):\n\tThread: %lu\n\tAppDomain: %u\n",
				pthread_self(), mono_domain_get()->domain_id);
	...
	*/
	...
	res = mono_mb_create_and_cache (cache, klass, mb, sig, sig->param_count + 16);
	mono_mb_free (mb);
...
mono_marshal_get_ldflda_wrapper (MonoType *type)
{
	...
	//  Beginn Pre-JIT/-Patch Code
	//
	if (snoopwrapper_org == 1) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_ldflda_wrapper () (MONO_WRAPPER_LDFLDA, Cache-Miss):\n\tThread: %lu\n\tAppDomain: %u\n",
				pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_stfld_remote_wrapper (MonoClass *klass)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_stfld_remote_wrapper () (MONO_WRAPPER_STFLD_REMOTE, Cache-Miss):\n\tKlasse: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				klass->name, pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_stfld_wrapper (MonoType *type)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_stfld_wrapper () (MONO_WRAPPER_STFLD, Cache-Miss):\n\tThread: %lu\n\tAppDomain: %u\n",
				pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_icall_wrapper (MonoMethodSignature *sig, const char *name, gconstpointer func, gboolean check_exceptions)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_icall_wrapper () (MONO_WRAPPER_MANAGED_TO_NATIVED):\n\tThread: %lu\n\tAppDomain: %u\n",
				pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_native_wrapper (MonoMethod *method, gboolean check_exceptions, gboolean aot)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_native_wrapper () (MONO_WRAPPER_MANAGED_TO_NATIVED, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
	...
	if (!(method->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) && (method->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL)) {
		pinvoke = TRUE;
	}
									  ...
									  MonoMethodPInvoke *piinfo, MonoMarshalSpec **mspecs, gpointer func)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_native_func_wrapper () (MONO_WRAPPER_MANAGED_TO_NATIVED, Cache-Miss):\n\tThread: %lu\n\tAppDomain: %u\n",
				pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_managed_wrapper (MonoMethod *method, MonoClass *delegate_klass, MonoObject **this_loc)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_managed_wrapper() (MONO_WRAPPER_NATIVE_TO_MANAGED, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_vtfixup_ftnptr (MonoImage *image, guint32 token, guint16 type)
{
	...
	//
	if (snoopwrapper_org == 1) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_vtfixup_ftnptr () (MONO_WRAPPER_NATIVE_TO_MANAGED):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_isinst (MonoClass *klass)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_isinst () (MONO_WRAPPER_ISINST, Cache-Miss):\n\tKlasse: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				klass->name, pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_castclass (MonoClass *klass)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_castclass () (MONO_WRAPPER_CASTCLASS, Cache-Miss):\n\tKlasse: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				klass->name, pthread_self(), mono_domain_get()->domain_id);
...
MonoMethod * mono_marshal_get_proxy_cancast (MonoClass *klass)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_proxy_cancast () (MONO_WRAPPER_PROXY_ISINST, Cache-Miss):\n\tKlasse: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				klass->name, pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_struct_to_ptr (MonoClass *klass)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_struct_to_ptr () (MONO_WRAPPER_UNKNOWN, Cache-Miss):\n\tKlasse: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				klass->name, pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_ptr_to_struct (MonoClass *klass)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_ptr_to_struct () (MONO_WRAPPER_UNKNOWN, Cache-Miss):\n\tKlasse: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				klass->name, pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_synchronized_wrapper (MonoMethod *method)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_synchronized_wrapper () (MONO_WRAPPER_SYNCHRONIZED, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_unbox_wrapper (MonoMethod *method)
{
	...
	//
	if (SNOOPWRAP) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_unbox_wrapper () (MONO_WRAPPER_UNBOX, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_stelemref ()
{
	...
	//
	if (SNOOPWRAP) {
		ret2 = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_stelemref () (MONO_WRAPPER_STELEMREF, Cache-Miss):\n\tThread: %lu\n\tAppDomain: %u\n",
				pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_array_address (int rank, int elem_size)
{
	...
	//
	if (snoopwrapper_org == 1) {
		ret2 = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_array_address () (MONO_WRAPPER_MANAGED_TO_MANAGED, Cache-Miss):\n\tThread: %lu\n\tAppDomain: %u\n",
				pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_generic_array_helper (MonoClass *class, MonoClass *iface, gchar *name, MonoMethod *method)
{
	...
	//
	if (snoopwrapper_org == 1) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_generic_array_helper () (MONO_WRAPPER_MANAGED_TO_MANAGED):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
...
mono_marshal_get_thunk_invoke_wrapper (MonoMethod *method)
{
	...
	//
	if (snoopwrapper_org == 1) {
		ret = asprintf(&messageToUser,"Mono-RT - mono_marshal_get_thunk_invoke_wrapper () (MONO_WRAPPER_NATIVE_TO_MANAGED, Cache-Miss):\n\tMethode: %s\n\tThread: %lu\n\tAppDomain: %u\n",
				mono_method_full_name(method, TRUE), pthread_self(), mono_domain_get()->domain_id);
